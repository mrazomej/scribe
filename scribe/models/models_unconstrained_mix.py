...
from numpyro.distributions import (
    constraints,
)  # Though not strictly used for unconstrained params, good to have if any part needs it.
...
def nbvcp_mixture_model_unconstrained(
    ...
    p = numpyro.deterministic(
        "p", jsp.special.expit(p_unconstrained_comp)
    )  # Shape (K,)
    r = numpyro.deterministic(
        "r", jnp.exp(r_unconstrained_comp)
    )  # Shape (K, G)
    ...
    with plate_context as idx:
        ...
        component_dist_batch = dist.NegativeBinomialProbs(
            ...
            probs=p_hat_batch[
                :, :, None
            ],  # Shape (B,K,1) -> broadcasts to (B,K,G)
        ).to_event(
            1
        )  # batch_shape=(B,K), event_shape=(G,)
...
def zinbvcp_mixture_model_unconstrained(
    ...
    with plate_context as idx:
        ...
        component_dist_batch = zinb_base_comp_dist.to_event(
            1
        )  # batch_shape=(B,K), event_shape=(G,)
