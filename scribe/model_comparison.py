...
def _compute_lppd(
    ...
    if aggregate:
        return jnp.sum(max_log_liks + jnp.log(jnp.mean(exp_centered, axis=0)))
    else:
...
def _compute_waic_stats(
    ...
    # Compute p_waic_2
    p_waic_2 = _compute_p_waic_2(log_liks, aggregate=aggregate, dtype=dtype)
...
def _compute_waic_weights(
    waic_values: jnp.ndarray, dtype: jnp.dtype = jnp.float32
) -> jnp.ndarray:
    """
