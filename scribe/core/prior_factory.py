...
class PriorConfigFactory:
    ...
    def _get_unified_defaults(
        ...
        if parameterization == "standard":
            ...
            if r_distribution is not None:
                ...
                dist_name = getattr(
                    r_distribution, "__name__", str(r_distribution)
                )
        ...
        if "mix" in model_type and n_components is not None:
            if parameterization == "unconstrained":
                defaults["mixing_prior"] = (
                    0,
                    1,
                )  # Normal on logits (simplified)
            else:
                defaults["mixing_prior"] = jnp.ones(
                    n_components
                )  # Symmetric Dirichlet
    ...
    def validate_priors(
        ...
        """
        PriorConfigFactory._validate_unified_priors(
            model_type, parameterization, prior_dict
        )

    @staticmethod
    def _validate_unified_priors(
        model_type: str, parameterization: str, prior_dict: Dict[str, Any]
    ) -> None:
        ...
        for prior_name in required_priors:
            ...
            if prior_name == "mixing_prior":
                ...
                if not isinstance(prior_value, (tuple, list, jnp.ndarray)):
                    raise ValueError(
                        f"mixing_prior must be a sequence, got {type(prior_value)}"
                    )
            else:
                # Standard tuple validation
                if not isinstance(prior_value, tuple) or len(prior_value) != 2:
                    raise ValueError(
                        f"{prior_name} must be a tuple of length 2, got {prior_value}"
                    )
