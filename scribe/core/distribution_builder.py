...
class DistributionBuilder:
    ...
    def _build_unconstrained_distributions(
        ...
        p_prior = priors["p_prior"]
        distributions["p_distribution_model"] = dist.Normal(
            p_prior[0], p_prior[1]
        )
        distributions["p_param_prior"] = p_prior
    ...
    def _build_gate_distributions(
        priors: Dict[str, Any], parameterization: str, inference_method: str
    ) -> Dict[str, Any]:
    ...
    def _build_capture_distributions(
        parameterization: str, priors: Dict[str, Any], inference_method: str
    ) -> Dict[str, Any]:
    ...
    def _build_mixing_distributions(
        ...
        if parameterization == "unconstrained":
            ...
            distributions["mixing_distribution_model"] = dist.Dirichlet(
                concentration
            )
            ...
            if inference_method == "svi":
                distributions["mixing_distribution_guide"] = dist.Dirichlet(
                    concentration
                )
...
def validate_distribution_priors(
    distribution_class: Type[dist.Distribution], prior_params: Tuple
) -> bool:
    """
