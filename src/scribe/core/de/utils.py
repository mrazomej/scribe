"""
Utility functions for Bayesian differential expression analysis.

This module provides Bayesian error control methods and result formatting
utilities for differential expression analysis.
"""
...
def compute_lfdr(
    ...
    The lfdr is the posterior probability that a gene is null (Δ=0) given
    the observed data. This uses an empirical Bayes approach.
    ...
    **WARNING**: This is a rough empirical Bayes approximation. For
    ...
    Notes
    -----
    lfdr_g = π₀·f₀(z_g) / f(z_g)

    where:
    - π₀ = prior probability of null
    - f₀(z) = N(0, s₀²) is null density
    - f(z) = mixture of null and alternative

    This is a rough empirical Bayes approximation. For more sophisticated
    implementations, consider fitting a two-group mixture model.

    For primary DE decisions, use lfsr from differential_expression()
    and control PEFP with find_lfsr_threshold().
...
def compute_pefp(
    ...
    Given a set of genes called DE (based on lfsr < threshold),
    the PEFP is the expected proportion of false discoveries:

    PEFP = E[# false discoveries / # discoveries | data]
         = Σ(lfsr for called genes) / (# called genes)
    ...
    Notes
    -----
    Unlike frequentist FDR, PEFP is a posterior quantity that directly
    uses the data to compute expected error rates.

    Examples
    --------
    >>> lfsr = jnp.array([0.01, 0.03, 0.5, 0.02, 0.8])
    >>> pefp = compute_pefp(lfsr, threshold=0.05)
    >>> print(f"Expected FDP: {pefp:.3f}")
    """
    ...
    # Sum of lfsr for called genes = expected # of false discoveries
...
def find_lfsr_threshold(
    ...
    This is the Bayesian version of FDR control: find the threshold
    such that the posterior expected FDP is at most the target.
    ...
    -------
    float
        Threshold value for lfsr that controls PEFP at target level
    ...
    >>> print(f"Use lfsr threshold: {threshold:.3f}")
    """
    # Sort lfsr values
    sorted_lfsr = jnp.sort(lfsr)
    n = len(lfsr)

    # For each possible threshold, compute PEFP
    # We test each unique lfsr value as a potential threshold
    thresholds = sorted_lfsr

    # Compute PEFP for each threshold
    pefps = jnp.array(
        [compute_pefp(lfsr, threshold=float(t)) for t in thresholds]
    )
...
def format_de_table(
    ...
    sort_by : str, default='lfsr'
        Column to sort by. Options: 'lfsr', 'delta_mean', 'prob_effect'
    top_n : int, optional
...
def _format_de_table_simple(
    ...
    for row in data:
        lines.append(
            f"{row[0]}\t{row[1]:.3f}\t{row[2]:.3f}\t{row[3]:.3f}\t{row[4]:.3f}\t{row[5]:.3f}"
        )
