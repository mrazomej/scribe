...
- CLR (Centered Log-Ratio): symmetric, D dimensional (constrained to simplex)
...
# ------------------------------------------------------------------------------
# ALR (Additive Log-Ratio) to CLR (Centered Log-Ratio) Transform
# ------------------------------------------------------------------------------
...
def alr_to_clr(z_alr: jnp.ndarray) -> jnp.ndarray:
    ...
    The CLR transformation centers the log-ratios:
        CLR_i = log(ρ_i) - (1/D) * sum_j log(ρ_j)
    ...
    z_full = jnp.concatenate([z_alr, jnp.zeros_like(z_alr[..., :1])], axis=-1)
...
def _exact_diag_after_centering(d_alr: jnp.ndarray) -> jnp.ndarray:
    ...
    For d_full = [d_alr, 0] and centering matrix C = I - (1/D)·1·1ᵀ,
    the diagonal of C·diag(d_full)·Cᵀ is:

        (C·diag(d)·Cᵀ)ᵢᵢ = d_fullᵢ·(1 - 2/D) + (1/D²)·Σⱼ d_fullⱼ

    This formula gives exact marginal variances after centering without
    materializing the centering matrix.
...
def transform_gaussian_alr_to_clr(
    ...
    For z_alr ~ N(μ, Σ) with Σ = W·Wᵀ + diag(d),
    compute exact parameters of z_clr = H·z_alr where H = C·E.

    Here:
        - E embeds (D-1)-dimensional ALR to D dimensions by appending 0
        - C = I - (1/D)·1·1ᵀ is the centering matrix

    The transformation preserves the low-rank structure and gives
    exact marginal variances (crucial for gene-level inference).
...
def build_ilr_basis(D: int) -> jnp.ndarray:
    ...
    The ILR basis V is a (D-1) × D matrix with orthonormal rows that are
    orthogonal to the CLR constraint (sum = 0).

    This implementation uses the Helmert contrast (sequential binary partition),
    where the i-th basis vector separates the first i+1 components from the
    rest.
    ...
    Notes
    -----
    The rows of V are orthonormal: V @ Vᵀ = I
...
# ------------------------------------------------------------------------------
# Project CLR to ILR (orthonormal basis)
# ------------------------------------------------------------------------------
...
def clr_to_ilr(clr: jnp.ndarray, V: jnp.ndarray) -> jnp.ndarray:
    ...
    ILR coordinates are orthonormal projections of CLR onto a (D-1)-dimensional
    basis. This transformation is an isometry (preserves distances).
    ...
    >>> assert ilr.shape == (2,)
    """
    return clr @ V.T  # (..., D) @ (D, D-1).T -> (..., D-1)


# ------------------------------------------------------------------------------
# Project ILR to CLR (orthonormal basis)
# ------------------------------------------------------------------------------
...
def ilr_to_clr(ilr: jnp.ndarray, V: jnp.ndarray) -> jnp.ndarray:
    ...
    >>> assert clr.shape == (3,)
    >>> assert jnp.allclose(clr.sum(), 0.0)
        ilr: ILR coordinates, shape (..., D-1)
        V: ILR basis matrix, shape (D-1, D)
    """
    return ilr @ V  # (..., D-1) @ (D-1, D) -> (..., D)
