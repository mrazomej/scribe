...
Parameters are sampled from appropriate prior distributions:
    ...
    - r is the dispersion parameter (computed via linked relationship)
    - The relationship is: r = mu * (1 - p) / p

This module implements the LINKED parameterization where:
    - p is the success probability (sampled from Beta prior)
    - mu is the mean expression (generated by VAE decoder)
    - r is computed via the linked relationship: r = mu * (1 - p) / p
"""
...
def nbdm_vae_model(
    ...
    model using linked parameterization suitable for variational inference
    ...
    The VAE architecture allows the model to:
        ...
        - Preserve the linked relationship between mean and dispersion
...
def nbdm_vae_guide(
    ...
    The VAE architecture provides several advantages:
        ...
        - Preservation of the linked parameterization structure
...
def nbvcp_vae_model(
    ...
    probability (NBVCP) using linked parameterization suitable for variational
    ...
    The model architecture consists of:
        1. Global parameters p and p_capture sampled from Beta prior distributions
        2. A latent variable z sampled from a standard Normal prior
        3. A decoder network that maps z to cell-specific mu parameters
        4. The linked parameterization that computes r from mu and p
...
def nbdm_dpvae_model(
    ...
    Binomial-Dirichlet Multinomial (NBDM) data using linked parameterization.
    ...
    The model architecture consists of:
        1. Global parameters p and mu sampled from appropriate prior distributions
...
def nbvcp_dpvae_model(
    ...
    variable mRNA capture probability (NBVCP) using linked parameterization.
    ...
    The model architecture consists of:
        1. Global parameters p, mu, and p_capture sampled from appropriate prior distributions
        2. A latent variable z sampled from a decoupled prior distribution
        3. A decoder network that maps z to cell-specific mu parameters
        4. The linked parameterization that computes r from mu and p
        5. Cell-specific capture probabilities that modify the effective success
...
def get_posterior_distributions(
    ...
    guide outputs for VAE-based models with linked parameterization.
    ...
    The function is designed to work with the linked parameterization where
    parameters are sampled from appropriate prior distributions (e.g., Beta for
    p, Normal for z) and the dispersion parameter r is computed via the linked
    relationship: r = mu * (1 - p) / p.
