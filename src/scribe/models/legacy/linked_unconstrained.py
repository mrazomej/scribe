...
def nbdm_model(
    ...
    p_prior_params = (
        model_config.priors.p
        if model_config.priors.p is not None
        else (
            0.0,
            1.0,
        )
...
def nbdm_guide(
    ...
    p_guide_params = (
        model_config.guides.p
        if model_config.guides.p is not None
        else (0.0, 1.0)
    )
    mu_guide_params = (
...
def nbvcp_model(
    ...
    p_capture_prior_params = (
        ...
        if model_config.priors.p_capture is not None
        else (
            0.0,
            1.0,
        )
...
def nbvcp_guide(
    ...
    p_capture_guide_params = (
        model_config.guides.p_capture
        if model_config.guides.p_capture is not None
        else (0.0, 1.0)
    )
...
@register(
    model_type="zinbvcp_mix", parameterization="linked", unconstrained=True
)
