...
# Correlation heatmap: ``n_genes`` / ``n_samples`` as keywords or under
# ``viz_cfg["heatmap_opts"]`` (keywords override config for new draws; for
# stored MCMC chains, ``n_samples`` caps draws used for correlations).
result = scribe.viz.plot_correlation_heatmap(
    results, counts, n_genes=500, n_samples=256, save=False
)
