...
``y = f(x)`` with ``x ~ p_X`` (typically Normal) and ``f`` a smooth
invertible transform, the *median* of ``y`` is ``f(loc_X)`` for monotone
``f``, but the *mode* (MAP) of ``y`` is **not** ``f(loc_X)``. The correct
constrained-space MAP requires a Jacobian (change-of-variables)
correction.

For ``X ~ p_X`` on ``R^n`` and elementwise ``f``, the density of
``Y = f(X)`` is

.. math::

    \\log p_Y(y) = \\log p_X(x) - \\log|J_f(x)|, \\quad x = f^{-1}(y),
...
For ``X ~ N(\\mu, \\Sigma)`` this becomes the coupled equation
...
``ExpTransform`` is ``x^* = \\mu - \\Sigma \\cdot \\mathbf{1}``, where
``\\Sigma \\cdot \\mathbf{1}`` is computed without ever materializing the
full ``n \\times n`` covariance using
``W (W^T \\mathbf{1}) + \\mathrm{cov\\_diag}``.
...
window of half-width ``K * max(\\sigma, \\sigma^2)`` centered at ``\\mu``.
For Sigmoid the unconstrained density is bimodal when ``\\sigma^2 > 2``
with modes asymptotically at ``\\mu \\pm \\sigma^2``; the adaptive grid
covers both regimes for ``\\sigma`` up to ``SIGMA_CEILING_WARN`` (default
...
``LowRankMVN + Exp`` handler can compute ``\\Sigma \\cdot \\mathbf{1}``
...
# NOTE on SlicedTransform: support for ``SlicedTransform`` (defined in
# ``scribe.flows.distributions``) is registered separately by
# ``scribe.flows`` to avoid a circular import (``scribe.flows`` ->
# ``scribe.models`` -> ``scribe.flows``). See ``_register_sliced_transform_handler``
# at the bottom of this module and ``scribe/flows/__init__.py``.

# ==============================================================================
# Constants
# ==============================================================================

#: Below this ``\\sigma`` floor, the Newton kernels would divide by huge
#: ``1/\\sigma^2`` terms and produce NaNs; we clip ``\\sigma`` here. Picked
#: small enough that the resulting correction is numerically
#: indistinguishable from ``transform(loc)`` in the limit.
_SIGMA_FLOOR = 1e-8

#: When ``\\sigma`` exceeds this ceiling, the adaptive-width grid may not
...
#: 21 points across a half-width of ``K * max(\\sigma, \\sigma^2)`` gives
#: a coarse argmin that the Newton refinement step polishes.
_DEFAULT_N_GRID = 21

#: Default multiplier for the grid half-width. Half-width is
#: ``_DEFAULT_GRID_WIDTH * max(\\sigma, \\sigma^2)``. The choice trades
...
#: in the supported ``\\sigma`` regime is achieved in 5-10 iterations; 30
...
def _map_unconstrained(t, base):  # noqa: F811
    ...
    For ``X ~ N(\\mu, \\sigma^2)`` and ``Y = e^X``, the density of ``Y`` is
    LogNormal with mode ``e^{\\mu - \\sigma^2}``. In unconstrained space:

    .. math::

        x^* = \\mu - \\sigma^2.

    Derivation: ``J'(x) = (x - \\mu)/\\sigma^2 + 1`` (since
...
def _map_unconstrained(t, base):  # noqa: F811
    ...
    For ``X ~ N(\\mu, \\Sigma)`` and ``Y = e^X`` (elementwise), the
    coupled stationarity condition is

    .. math::

        \\Sigma^{-1}(x^* - \\mu) + \\mathbf{1} = 0
        \\;\\Longrightarrow\\;
        x^* = \\mu - \\Sigma \\cdot \\mathbf{1}.

    For ``\\Sigma = W W^T + \\mathrm{diag}(\\mathrm{cov\\_diag})``, the
    quantity ``\\Sigma \\cdot \\mathbf{1}`` is computed cheaply via

    .. math::

        \\Sigma \\cdot \\mathbf{1} = W (W^T \\mathbf{1}) +
            \\mathrm{cov\\_diag}
    ...
    # W^T @ 1_n: column sums of W, giving a k-vector per leading batch.
    w_sum = W.sum(axis=-2)  # (..., k)
    # W @ (W^T @ 1_n): rank-k contribution to ``Sigma @ 1_n``.
    Sigma_dot_1 = (
...
def _map_unconstrained(t, base):  # noqa: F811
    ...
    For ``X ~ N(\\mu, \\sigma^2)`` and ``Y = \\log(1 + e^X)``, the
    objective to minimize (negative log-density of ``Y``, ignoring
    constants) is

    .. math::

        J(x) = \\frac{(x - \\mu)^2}{2 \\sigma^2}
             - \\log \\sigma(x),

    where ``\\sigma(x)`` is the sigmoid function (the Jacobian of
    softplus). No closed form exists; we use adaptive grid + Newton
    refinement (see :func:`_grid_plus_newton`).
    """
...
def _map_unconstrained(t, base):  # noqa: F811
    ...
    For ``X ~ N(\\mu, \\sigma^2)`` and ``Y = \\sigma(X)``, the objective
    is
    ...
    The constrained density is **bimodal** when ``\\sigma^2 > 2``, with
    modes asymptotically at ``x \\approx \\mu \\pm \\sigma^2`` for large
    ``\\sigma``. A single-start Newton at ``\\mu`` would land on a saddle
    in that regime; the adaptive grid (half-width
    ``K \\cdot \\max(\\sigma, \\sigma^2)``) covers both modes and the
    Newton refinement polishes the argmin.
    """
    return _grid_plus_newton(
        J_fn=_J_sigmoid,
...
def _map_unconstrained(t, base):  # noqa: F811
    ...
    ``\\Sigma^{-1}(x^* - \\mu) + (1 - \\sigma(x^*)) = 0``, which is a
...
# NOTE: SlicedTransform handler is registered by scribe.flows at its
# load time (see ``_register_sliced_transform_handler`` below and the
# call in ``scribe/flows/__init__.py``). This avoids a circular import.
...
# are floored from below at ``0.25 / \\sigma^2`` to prevent Newton blowup
...
def _safe_sigma(sigma):
    """Clip ``sigma`` to a small positive floor to prevent ``1/\\sigma^2``
    ...
    in the ``\\sigma \\to 0`` limit (where median equals mode anyway).
    """
...
def _J_softplus(x, mu, sigma):
    ...
    .. math::
        ...
        J(x) = -\\log p_Y(y) = -\\log p_X(x) + \\log|f'(x)|
             = \\frac{(x - \\mu)^2}{2 \\sigma^2} + \\log \\sigma(x).

    Note: ``log|f'(x)|`` for Softplus equals ``log \\sigma(x)``. The
    sign is ``+``, not ``-``: a small Jacobian (``f'(x) \\to 0`` as
    ``x \\to -\\infty``) means ``Y`` covers a small chunk of the
    constrained space per unit ``X``, so ``Y``'s density there is
    LARGER. This is why the Softplus mode shifts *negative* from
    ``\\mu`` (toward small-Jacobian region).
...
def _newton_step_softplus(x, mu, sigma):
    ...
    Gradient: ``J'(x) = (x - \\mu)/\\sigma^2 + (1 - \\sigma(x))``.
    Hessian:  ``J''(x) = 1/\\sigma^2 - \\sigma(x)(1 - \\sigma(x))``.

    Since ``\\sigma(1 - \\sigma) \\le 1/4``, ``J''(x) > 0`` whenever
    ``\\sigma^2 < 4``. Outside that regime the Hessian can dip slightly
    negative; the floor at ``0.25 / \\sigma^2`` keeps the Newton step
...
def _J_sigmoid(x, mu, sigma):
    r"""Negative log-density of ``Y = \\sigma(X)`` (ignoring constants).
    ...
    ``\\log|f'(x)| = \\log(\\sigma(x) (1 - \\sigma(x)))`` in a way that
    ...
    Bimodality: ``J''(x) = 1/\\sigma^2 - 2 \\sigma(x)(1 - \\sigma(x))``
    can be negative (``J`` non-convex) when ``\\sigma^2 > 2``; the
    central stationary point at ``\\mu`` then becomes a local MAX of
    ``J`` (= local MIN of density), with two true modes at
    ``x \\approx \\mu \\pm \\sigma^2``. The grid+Newton scaffold finds
...
def _newton_step_sigmoid(x, mu, sigma):
    ...
    Gradient: ``J'(x) = (x - \\mu)/\\sigma^2 + (1 - 2 \\sigma(x))``.
    Hessian:  ``J''(x) = 1/\\sigma^2 - 2 \\sigma(x)(1 - \\sigma(x))``.

    In the convex regime (``\\sigma^2 < 2``), ``J''(x) > 0`` everywhere
    and Newton from ``\\mu`` converges to the unique mode. In the
    bimodal regime, ``J''`` can be negative near ``\\mu``; the Hessian
    floor at ``0.25 / \\sigma^2`` keeps the step bounded. Global mode
...
def _grid_plus_newton(
    *,
    ...
    1. Compute an adaptive grid half-width per element:
       ``W(\\sigma) = K \\cdot \\max(\\sigma, \\sigma^2)``. This covers
       both the Gaussian-dominated regime (``\\sigma < 1``, modes near
       ``\\mu \\pm K \\sigma``) and the Jacobian-dominated regime
       (``\\sigma > 1``, modes near ``\\mu \\pm K \\sigma^2`` for
       Sigmoid).
    2. Sample ``n_grid`` candidate points uniformly across
       ``[\\mu - W, \\mu + W]`` per element.
    ...
    Heuristic caveat
    ----------------
    The half-width ``K \\cdot \\max(\\sigma, \\sigma^2)`` is wide enough
    for ``\\sigma`` up to ``SIGMA_CEILING_WARN``; beyond that, asymptotic
    modes can escape the grid. The public wrapper checks ``\\sigma``
    ...
    sigma_safe = _safe_sigma(sigma)
    # Half-width adapts to whichever regime dominates: \\sigma for tight
    # posteriors, \\sigma^2 for wide ones where the Jacobian shifts the
    # mode far from \\mu.
...
def _grid_plus_newton_autodiff(
    ...
    def J_fn(x, mu_arr, sigma_arr):
        ...
        return jax.vmap(J_scalar)(x.ravel(), mu_arr.ravel(), sigma_arr.ravel()
                                  ).reshape(x.shape)
...
def jacobian_corrected_map(
    ...
    *,
    ...
    if method == "newton":
        ...
        # Sigmoid), or SlicedTransform (recurses to slice-level newton
        # compatibility; detected via duck-typing to avoid an explicit
        # dependency on scribe.flows here — see SlicedTransform note
        ...
        ) or _is_sliced_transform(transform)
...
def _extract_scale_for_warning(base) -> Optional[jnp.ndarray]:
    ...
    if isinstance(base, LowRankMultivariateNormal):
        # Marginal stddevs: sqrt(diag(WW^T) + cov_diag)
        W = base.cov_factor
...
# ==============================================================================
# SlicedTransform support — registered lazily by scribe.flows
# ==============================================================================
#
# ``SlicedTransform`` lives in ``scribe.flows.distributions``, which has
# transitive dependencies on ``scribe.models`` that create a circular
# import if we try to ``from scribe.flows.distributions import SlicedTransform``
# at the top of this module. Instead, we expose a registration function
# that ``scribe.flows.__init__`` calls after SlicedTransform is defined.


# Module-level flag so we don't double-register if ``scribe.flows`` is
# imported multiple times (e.g., in test setups).
_SLICED_TRANSFORM_REGISTERED = False


def _is_sliced_transform(t) -> bool:
    """Duck-typed check for ``SlicedTransform`` without importing it.

    Used in the ``method='newton'`` gate to allow SlicedTransform-wrapped
    cases through to the dispatched handler (which itself enforces that
    each slice is newton-compatible). Avoids importing
    ``scribe.flows.distributions`` from this module.

    The check is conservative: it requires all three internal attributes
    that SlicedTransform exposes (``_transforms``, ``_offsets``,
    ``_sizes``). False positives would only arise for transforms that
    happen to expose the same triple — none in scribe.
    """
    return (
        hasattr(t, "_transforms")
        and hasattr(t, "_offsets")
        and hasattr(t, "_sizes")
    )


def _register_sliced_transform_handler() -> None:
    """Register the ``SlicedTransform`` dispatch handler.

    Called by ``scribe.flows.__init__`` after ``SlicedTransform`` is
    defined. Idempotent — re-registration is a no-op.

    The handler performs per-slice recursion: each slice's MAP
    correction is computed independently (the Jacobian of a
    SlicedTransform is block-diagonal, so slices decouple), and the
    pieces are concatenated along the trailing axis. ``Normal`` and
    ``Independent(Normal)`` bases are supported via :func:`_slice_base`;
    ``LowRankMultivariateNormal`` raises because slicing breaks the
    rank-``k`` factorisation.
    """
    global _SLICED_TRANSFORM_REGISTERED
    if _SLICED_TRANSFORM_REGISTERED:
        return

    # Local import: at this point scribe.flows is loaded enough that the
    # SlicedTransform class is in the namespace.
    from scribe.flows.distributions import SlicedTransform

    # IMPORTANT: this handler MUST be named ``_map_unconstrained`` (not
    # something else) so that multipledispatch extends the existing
    # ``_map_unconstrained`` dispatcher rather than creating a new one
    # keyed by a different name. ``dispatch`` uses ``__name__`` to look
    # up the dispatcher registry.
    @dispatch(SlicedTransform, Distribution)
    def _map_unconstrained(t, base):  # noqa: F811
        """Per-slice MAP recursion for SlicedTransform. See
        :func:`_register_sliced_transform_handler` for details."""
        if isinstance(base, LowRankMultivariateNormal):
            raise NotImplementedError(
                "SlicedTransform over LowRankMultivariateNormal is not "
                "supported in v1; slicing a low-rank covariance breaks "
                "the rank-k factorisation. Use map_method='transform' "
                "instead."
            )
        pieces = []
    ...
    _SLICED_TRANSFORM_REGISTERED = True
