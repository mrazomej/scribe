...
def fit(
    ...
    # "Rung 1.5") is declared through the unified ``priors`` dict, e.g.
    # ``priors={"module_weight": {"sample": "gaussian",
    # "perturbation:sample": "gaussian"}}``, reusing the ``hierarchy`` /
    # ``interactions`` grouping.  There is no dedicated ``fit`` kwarg.  See
