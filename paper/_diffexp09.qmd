...
closure. Because the centered log-ratio (CLR) transform uses the geometric
mean of all retained genes as its reference, the choice of mask directly
defines the yardstick against which effects are measured. This section
formalises the consequences.

**Setup.** Let $\boldsymbol\rho^{(s)}$ denote a simplex sample of dimension
$D$, partitioned into kept genes $\mathcal{K}$ ($|\mathcal{K}| = D_k$) and
filtered genes $\mathcal{F}$. Write $\rho_{\mathrm{other}}^{(s)} =
\sum_{g \in \mathcal{F}} \rho_g^{(s)}$ for the aggregated "other"
proportion. The masked simplex has $D_k + 1$ components.
...
geometric mean. For each posterior sample, the difference between the
full-gene CLR and the masked CLR for a kept gene is a _per-sample constant_:
...
where $\bar{g}_{\mathrm{mask}}^{(s)}$ and $\bar{g}_{\mathrm{full}}^{(s)}$
are the geometric means of the masked and full compositions, respectively.
This shift $c^{(s)}$ is the same for every gene $g \in \mathcal{K}$ in a
given sample.

**Why the shift matters.** Although $\text{Var}[c^{(s)}]$ is typically
$O(1/D)$ (the shift is tight across samples), its expectation
$\langle c^{(s)} \rangle$ can be large. In log-space, low-expression genes
dominate the geometric mean because their $\log \rho_g$ values are large
negative numbers. When many genes are low-expression (common in
transcriptomics), the full-gene geometric mean is pulled strongly downward
relative to the masked geometric mean that only includes expressed genes.
This produces a substantial constant offset in $c^{(s)}$ that shifts all
$\Delta_g^{(s)}$ uniformly.

Although the shift is constant across genes within each sample, the
magnitude and sign of the shift vary across posterior samples, so it does
not cancel when averaging. The resulting bias completely scrambles the local
false sign rate (lfsr): genes that should have lfsr $\approx 0$ acquire
spurious probability mass on the wrong sign, and vice versa. **Therefore,
the expression mask must be applied before the CLR transform, not as a
post-hoc filter on the CLR differences.**

**Dirichlet aggregation property.** Fortunately, the Dirichlet distribution
has a closure property under aggregation: if $\boldsymbol\rho \sim
\mathrm{Dir}(\boldsymbol\alpha)$, then the composition formed by summing
any subset of components into a single entry is itself Dirichlet with the
summed concentration. Formally, if we aggregate $\mathcal{F}$ into "other":
...
initial Dirichlet draw and re-aggregate post-hoc for any gene mask—the result
is _mathematically identical_ to having pre-aggregated the concentration
parameters and sampled the lower-dimensional Dirichlet. This enables
interactive mask exploration on a single results object without re-running
the expensive Dirichlet/Gamma sampling stage.
