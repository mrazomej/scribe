...
**Computational advantage.** Because $\underline{\underline{V}}$ is orthonormal,
the ILR coordinates are uncorrelated:

$$
\text{Cov}(\Delta_{\text{ILR},i}, \Delta_{\text{ILR},j}) = 0 \quad 
\text{for } i \neq j.
$${#eq-diffexp-ilr-uncorrelated}

This orthogonality simplifies interpretation: testing whether
$\Delta_{\text{ILR},i}$ is non-zero is independent of the values of other ILR
coordinates. In contrast, testing individual gene-level effects
$\Delta_{\text{CLR},g}$ in CLR space involves correlated variables, which can
complicate multiple testing adjustments.
