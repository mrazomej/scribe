...
proportions. The factorization is at the mRNA level (shared $p$, $\theta =
p/(1-p)$); we learn $\underline{r}$, $p$, and $\nu$, and $\hat{p}$ is derived. The
normalization (composition) is justified at the mRNA level and depends only on
$\underline{r}$; the same structure applies to UMI counts (total
$\text{NB}(r_T, \hat{p})$, composition DM($\underline{r}$)). A key result from
that derivation is that normalized expression
...
\underline{\underline{U}})$ (we learn $\underline{r}$, $p$, and $\nu$; $\hat{p}$
is derived and used to deconvolve capture from biology), where $\underline{\underline{U}}$ represents the
