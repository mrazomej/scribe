...
**Gaussianity diagnostics and non-parametric DE** (Section 9). We developed
tools to assess and address violations of the Gaussian assumption:

- Per-gene Gaussianity diagnostics (skewness, excess kurtosis, Jarque-Bera
statistic) computed in a single $O(ND)$ vectorized pass
(@eq-diffexp-np-skewness through @eq-diffexp-np-jarque-bera)
- A non-parametric empirical DE path that computes all gene-level statistics
(lfsr, tail probabilities, practical significance) by direct Monte Carlo
counting over posterior CLR difference samples, requiring no distributional
assumptions (@eq-diffexp-np-lfsr and @eq-diffexp-np-lfsr-tau)
- A rigorous proof that index-pairing of independent posterior samples yields
valid posterior estimates of the difference distribution
(@eq-diffexp-np-posterior-independence through @eq-diffexp-np-permuted-delta)
- Full compatibility of empirical lfsr values with the existing PEFP error
control framework, enabling a hybrid parametric/empirical workflow
...
**Gaussian assumption.** The parametric DE framework (Sections 1--8) rests on
the Gaussian approximation to the posterior predictive distribution of
normalized expression. As discussed in @sec-normalization, this approximation is
justified when concentration parameters are moderate to large, but may be less
accurate for genes with very low expression or in settings with extreme
sparsity. In @sec-diffexp-nonparametric, we address this limitation directly:
per-gene Gaussianity diagnostics identify genes for which the approximation
breaks down, and a non-parametric empirical DE path provides assumption-free
inference for such genes. The two approaches can be combined in a hybrid
workflow where each gene is analyzed by the most appropriate method.
...
@sec-diffexp-nonparametric computes lfsr by direct counting over posterior
samples. Future refinements could include FFT-based kernel density estimation
(KDE) for smoother density estimates with adaptive bandwidths, which would
improve lfsr resolution without increasing the number of posterior samples
...
### Gaussianity Diagnostics and Non-Parametric Methods
...
normality. Empirical results for the distributions of $b_2$ and
$\sqrt{b_1}$." *Biometrika*, 60(3), 613-622. [Skewness and kurtosis tests]
