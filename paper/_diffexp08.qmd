...
The empirical CLR pipeline and its biological-level complements, developed in
the preceding sections, treat each gene as an isolated inference problem. This
section develops an **optional** empirical Bayes shrinkage layer that uses the
genome-wide distribution of effect sizes to refine per-gene estimates. By
learning the population-level prior from all $D$ genes simultaneously, the
shrinkage posterior pulls noisy effect estimates toward zero, potentially
improving calibration for genes near the decision boundary.

We present the full mathematical theory below---the scale mixture of normals
prior, the EM algorithm for learning mixture weights, and the closed-form
shrinkage posterior. However, we note at the outset that practical experience
across multiple datasets suggests that the shrinkage layer can be **overly
conservative**: the scale mixture prior, which concentrates substantial mass
near zero, tends to shrink genuine moderate effects too aggressively, leading to
missed discoveries. **The unshrunk empirical lfsr from the direct Monte Carlo
pipeline (@sec-diffexp-empirical) is therefore the recommended default.** The
shrinkage layer is best used as a secondary analysis for comparison, or in
settings where an explicit genome-wide null fraction estimate is desired.

The empirical differential expression framework developed in
@sec-diffexp-empirical treats each gene as an isolated inference problem. For
each gene $g$, we obtain a posterior mean effect $\hat{\Delta}_g$ and a
...
After running the empirical pipeline from @sec-diffexp-empirical, we have, for
each gene $g = 1, 2, \ldots, D$:

- A posterior mean CLR difference $\hat{\Delta}_g = \frac{1}{N}\sum_{s=1}^{N}
  \Delta_g^{(s)}$.
- A posterior standard deviation $s_g =
  \sqrt{\frac{1}{N-1}\sum_{s=1}^{N}(\Delta_g^{(s)} - \hat{\Delta}_g)^2}$.
...
Consider now the collection of all $D$ pairs $\{(\hat{\Delta}_1, s_1),
(\hat{\Delta}_2, s_2), \ldots, (\hat{\Delta}_D, s_D)\}$. In a typical
differential expression analysis, the vast majority of genes are not
differentially expressed: their true CLR effects are zero or negligibly small.
Only a small fraction exhibit genuine biological differences between conditions.
This global structure is visible in the histogram of $\hat{\Delta}_g$ values,
which typically shows a large central peak (the null genes) with sparse tails
(the truly DE genes).
...
$$
\beta_g \sim \pi(\beta), \quad g = 1, 2, \ldots, D, $${#eq-diffexp-eb-prior}
...
$$
\pi(\beta \mid \underline{w}) = \sum_{k=0}^{K} w_k \, \mathcal{N}(\beta \mid 0,
\sigma_k^2), $${#eq-diffexp-eb-scale-mixture}

where:

- $\sigma_0, \sigma_1, \ldots, \sigma_K$ is a **fixed grid** of scale parameters
  with $\sigma_0 \approx 0 < \sigma_1 < \sigma_2 < \cdots < \sigma_K$.
- $\underline{w} = (w_0, w_1, \ldots, w_K)$ is a vector of **mixture weights**
  satisfying $w_k \geq 0$ and $\sum_{k=0}^{K} w_k = 1$.
- The component $k = 0$ with $\sigma_0 \approx 0$ approximates a point mass at
  zero: $\mathcal{N}(\beta \mid 0, \sigma_0^2) \approx \delta_0(\beta)$. This is
  the **null component** representing genes with no differential expression.

The grid of scales $\sigma_0, \ldots, \sigma_K$ is fixed in advance (we describe
the default construction in @sec-diffexp-eb-complexity). Only the weights
$\underline{w}$ are estimated from the data. The weight $w_0$ on the null
component is of particular scientific interest: it estimates the fraction of
genes that are truly non-DE.

The prior is **unimodal at zero** (encoding symmetry between up- and
down-regulation), **conjugate** with the Gaussian observation model
(@eq-diffexp-eb-observation) so that all posterior computations have closed-form
solutions, and **flexible**: a fine grid of scales can approximate any unimodal
symmetric distribution to arbitrary accuracy.
...
\int_{-\infty}^{\infty} \mathcal{N}(\hat{\Delta}_g \mid \beta_g, s_g^2) \cdot
...
$$
\mathcal{N}(\beta_g \mid 0, \sigma_k^2) = \frac{1}{\sqrt{2\pi \sigma_k^2}}
...
$$
= -\frac{1}{2}\left[ \beta_g^2 \left(\frac{1}{s_g^2} +
\frac{1}{\sigma_k^2}\right)
- 2 \beta_g \frac{\hat{\Delta}_g}{s_g^2}
+ \frac{\hat{\Delta}_g^2}{s_g^2} \right]. $${#eq-diffexp-eb-exponent-grouped}
...
$$
v_{gk} = \frac{1}{\tau_{gk}} = \frac{\sigma_k^2 \, s_g^2}{\sigma_k^2 + s_g^2}.
$${#eq-diffexp-eb-posterior-variance}

The posterior mean is obtained by completing the square in the exponent. The
coefficient of $\beta_g$ in @eq-diffexp-eb-exponent-grouped is $\hat{\Delta}_g /
s_g^2$. Dividing by the coefficient of $\beta_g^2$ (which is $\tau_{gk}$) gives
the posterior mean:
...
$$
-\frac{1}{2}\left[ \tau_{gk} (\beta_g - m_{gk})^2
+ \hat{\Delta}_g^2 \left( \frac{1}{s_g^2} - \frac{1}{s_g^2} \cdot
    \frac{\sigma_k^2}{\sigma_k^2 + s_g^2} \right) \right].
...
$$
-\frac{1}{2}\left[ \tau_{gk} (\beta_g - m_{gk})^2
+ \frac{\hat{\Delta}_g^2}{\sigma_k^2 + s_g^2} \right].
...
Integrating over $\beta_g$, the Gaussian kernel contributes $\sqrt{2\pi
v_{gk}}$, and the prefactors from @eq-diffexp-eb-gaussian-likelihood and
@eq-diffexp-eb-gaussian-prior contribute $(2\pi s_g^2)^{-1/2} (2\pi
\sigma_k^2)^{-1/2}$. Combining:
...
$$
\boxed{ f(\hat{\Delta}_g \mid \underline{w}) = \sum_{k=0}^{K} w_k \,
...
(the grid $\sigma_0, \ldots, \sigma_K$ is fixed). We estimate $\underline{w}$ by
maximizing the **marginal log-likelihood** across all $D$ genes:
...
$$
c_g \sim \text{Categorical}(\underline{w}), $${#eq-diffexp-eb-latent-indicator}
...
$$
\mathcal{L}_{\text{complete}}(\underline{w}) = \sum_{g=1}^{D} \left[ \log
w_{c_g} + \log \mathcal{N}(\hat{\Delta}_g \mid 0, \sigma_{c_g}^2 + s_g^2)
\right]. $${#eq-diffexp-eb-complete-loglik}
...
w_k^{(t)} \cdot \mathcal{N}(\hat{\Delta}_g \mid 0, \sigma_k^2 + s_g^2).
...
$$
\boxed{ \gamma_{gk}^{(t)} = \frac{ w_k^{(t)} \cdot \mathcal{N}(\hat{\Delta}_g
\mid 0, \sigma_k^2 + s_g^2) }{ \sum_{j=0}^{K} w_j^{(t)} \cdot
\mathcal{N}(\hat{\Delta}_g \mid 0, \sigma_j^2 + s_g^2) }. }
...
construction, $\gamma_{gk}^{(t)} \geq 0$ and $\sum_{k=0}^{K} \gamma_{gk}^{(t)} =
1$ for each $g$.
...
$$
\sum_{g=1}^{D} \sum_{k=0}^{K} \gamma_{gk}^{(t)} \log w_k = \sum_{k=0}^{K}
\left(\sum_{g=1}^{D} \gamma_{gk}^{(t)}\right) \log w_k = \sum_{k=0}^{K}
N_k^{(t)} \log w_k, $${#eq-diffexp-eb-mstep-objective}

where $N_k^{(t)} = \sum_{g=1}^{D} \gamma_{gk}^{(t)}$ is the effective number of
genes assigned to component $k$. Note that $\sum_{k=0}^{K} N_k^{(t)} = D$.
...
$$
\frac{\partial \mathcal{J}}{\partial w_k} = \frac{N_k^{(t)}}{w_k} - \lambda = 0
\quad \implies \quad w_k = \frac{N_k^{(t)}}{\lambda}.
...
$$
\boxed{ w_k^{(t+1)} = \frac{N_k^{(t)}}{D} = \frac{1}{D} \sum_{g=1}^{D}
\gamma_{gk}^{(t)}. } $${#eq-diffexp-eb-mstep-result}
...
The EM algorithm alternates between the E-step (@eq-diffexp-eb-estep-result) and
the M-step (@eq-diffexp-eb-mstep-result) until the change in the log-likelihood
$|\mathcal{L}(\underline{w}^{(t+1)}) - \mathcal{L}(\underline{w}^{(t)})|$ falls
below a tolerance $\varepsilon$, or a maximum number of iterations is reached.
...
\mathcal{L}(\underline{w}^{(t)})$. This follows from Jensen's inequality applied
to the concavity of the logarithm. Since $\mathcal{L}$ is also bounded above
(being a log-likelihood of a normalized density), the sequence
...
Having estimated the mixture weights $\underline{w}^*$ via EM, we now derive the
posterior distribution of each gene's true effect $\beta_g$ given its observed
effect $\hat{\Delta}_g$.
...
$$
\pi(\beta_g \mid \hat{\Delta}_g, \underline{w}^*) = \frac{
\mathcal{N}(\hat{\Delta}_g \mid \beta_g, s_g^2) \cdot \sum_{k=0}^{K} w_k^* \,
\mathcal{N}(\beta_g \mid 0, \sigma_k^2) }{ \sum_{j=0}^{K} w_j^* \,
\mathcal{N}(\hat{\Delta}_g \mid 0, \sigma_j^2 + s_g^2) }.
...
$$
\boxed{ \pi(\beta_g \mid \hat{\Delta}_g, \underline{w}^*) = \sum_{k=0}^{K}
\gamma_{gk}^* \, \mathcal{N}(\beta_g \mid m_{gk}, v_{gk}), }
...
- **Null component** ($\sigma_k \approx 0$): $\alpha_{gk} \approx 0$, so $m_{gk}
  \approx 0$. The posterior mean is shrunk all the way to zero. Gene $g$'s
  observed effect is entirely attributed to noise.
...
$$
\langle \beta_g \mid \hat{\Delta}_g, \underline{w}^* \rangle =
...
$$
\text{Var}[\beta \mid \text{data}] = \underbrace{\sum_{k} \gamma_k \,
v_k}_{\text{within-component variance}}
...
@sec-diffexp-empirical. The most important is the **shrunk local false sign
rate**.
...
$$
P(\beta_g > 0 \mid \hat{\Delta}_g, \underline{w}^*) = \sum_{k=0}^{K}
\gamma_{gk}^* \int_0^{\infty} \mathcal{N}(\beta_g \mid m_{gk}, v_{gk}) \,
d\beta_g. $${#eq-diffexp-eb-prob-positive-sum}

The integral $\int_0^{\infty} \mathcal{N}(\beta_g \mid m, v) \, d\beta_g$ is the
probability that a $\mathcal{N}(m, v)$ random variable exceeds zero, which is
...
$$
\boxed{ P(\beta_g > 0 \mid \hat{\Delta}_g, \underline{w}^*) = \sum_{k=0}^{K}
\gamma_{gk}^* \, \Phi\!\left(\frac{m_{gk}}{\sqrt{v_{gk}}}\right). }
...
$$
P(\beta_g > \tau \mid \hat{\Delta}_g, \underline{w}^*) = \sum_{k=0}^{K}
\gamma_{gk}^* \, \Phi\!\left(\frac{m_{gk} - \tau}{\sqrt{v_{gk}}}\right),
...
$$
\text{lfsr}_g^{\text{shrunk}}(\tau) = 1 - \max\!\left( P(\beta_g > \tau \mid
\hat{\Delta}_g, \underline{w}^*), \; P(\beta_g < -\tau \mid \hat{\Delta}_g,
\underline{w}^*) \right). $${#eq-diffexp-eb-lfsr-tau}
...
@sec-diffexp-pefp operates exclusively on these numerical values---it does not
depend on how they were computed. Therefore:
...
dominant components are the large-scale ones with $\alpha_{gk} \approx 1$. The
shrunk posterior mean $\langle \beta_g \mid \hat{\Delta}_g \rangle \approx
\hat{\Delta}_g$, and the shrunk lfsr converges to the raw empirical lfsr. The
shrinkage layer becomes a no-op, ensuring backward compatibility.
...
**Default scale grid construction.** Given the vector of observed standard
errors $s_1, s_2, \ldots, s_D$, a default grid of $K + 1$ scales is constructed
...
$$
\pi(\beta_g \mid \hat{\Delta}_g, \underline{w}^*) = \sum_{k=0}^{K} \gamma_{gk}^*
\, \mathcal{N}(\beta_g \mid m_{gk}, v_{gk}),
$${#eq-diffexp-eb-summary-posterior}

with $m_{gk} = \hat{\Delta}_g \sigma_k^2 / (\sigma_k^2 + s_g^2)$ and $v_{gk} =
\sigma_k^2 s_g^2 / (\sigma_k^2 + s_g^2)$.
...
::: {.callout-warning}
## Practical note on shrinkage conservatism

In our experience across diverse single-cell datasets, the empirical Bayes shrinkage layer tends to be overly conservative. The scale mixture prior allocates substantial weight to the near-zero component, which pulls moderate but genuine effects toward the null. This produces higher lfsr values and fewer discoveries compared to the unshrunk empirical pipeline.

We recommend the following workflow:

1. **Default**: Use the unshrunk empirical lfsr from @sec-diffexp-empirical as the primary analysis.
2. **Comparison**: Run the shrinkage layer and compare the shrunk vs. unshrunk gene lists. Genes called by both methods are high-confidence discoveries; genes called only by the unshrunk pipeline deserve closer inspection but should not be dismissed.
3. **Null fraction**: Use the estimated weight $w_0^*$ on the null component as a descriptive summary of the genome-wide treatment effect, even when using the unshrunk lfsr for gene calling.
:::
