...
RNA-seq UMI counts. The shared $p$ (mRNA-level) assumption underlies that
derivation; $\hat{p}$ is the effective parameter after capture, used to
deconvolve experimental noise from biology. A key step in that derivation was
showing that the composition of a negative binomial distribution (representing
true transcript counts) with a binomial distribution (representing the
stochastic capture process) yields another negative binomial distribution with
an effective parameter $\hat{p}$. The variable capture probability (VCP) model further allows
...
the shared success probability (at the mRNA level). Recall from @sec-dirichlet-multinomial that the
...
Inferring $p$ and $\nu$ (hence $\hat{p}$) separates experimental noise from
underlying biology. This is not a coincidence---the same algebraic structure that relates the
