...
## Bayesian Error Control {#sec-diffexp-pefp}

The empirical CLR pipeline developed in the preceding section produces a local
false sign rate (lfsr) for every gene in the genome. Each lfsr$_g$ quantifies
the posterior probability that gene $g$'s differential expression has been
assigned the wrong sign---a fully Bayesian measure of evidence that requires no
distributional assumptions. With $D \approx 30{,}000$ genes, however, we need a
principled approach to decide which genes to declare as differentially expressed
while controlling the overall error rate.

In this section, we develop the posterior expected false discovery proportion
(PEFP), a Bayesian analog of the false discovery rate, and derive an efficient
algorithm for finding lfsr thresholds that control PEFP at a specified level.
The PEFP framework operates exclusively on numerical lfsr values and is agnostic
to how they were computed---whether by empirical counting over posterior
samples, by evaluation of the Gaussian CDF in the parametric path, or by the
empirical Bayes shrinkage layer. This modularity is a deliberate design feature:
any method that produces valid lfsr values can feed directly into the error
control machinery developed here.
...
Suppose we compute lfsr values for all $D$ genes, yielding $\{\text{lfsr}_1,
\text{lfsr}_2, \ldots, \text{lfsr}_D\}$. We wish to select a subset $S \subseteq
\{1, 2, \ldots, D\}$ of genes to declare as differentially expressed, based on
some criterion. A natural criterion is to threshold the lfsr: genes with
$\text{lfsr}_g < t$ for some threshold $t$ are declared significant.
...
$$
\text{FDR} = \langle \frac{V}{R} \rangle,
...
$$
V_g = \begin{cases} 1 & \text{if gene } g \text{ is a false discovery}, \\
0 & \text{if gene } g \text{ is a true discovery}. \end{cases}
...
$$
\text{PEFP}(S) = \langle \text{FDP}(S) \mid \text{data} \rangle = \begin{cases}
\frac{1}{|S|} \sum_{g \in S} \langle V_g \mid \text{data} \rangle & \text{if }
|S| > 0, \\
0 & \text{if } |S| = 0.
\end{cases} $${#eq-diffexp-pefp-definition}

#### Approximating PEFP with lfsr

To compute $\langle V_g \mid \text{data} \rangle$, we need to define what
...
$$
\langle V_g \mid \text{data} \rangle = P(\text{wrong sign for gene } g \mid \text{data}) = 
\text{lfsr}_g.
$${#eq-diffexp-ev-lfsr}
...
$$
\text{lfsr}_g = \min\left(P(\Delta_g > 0 \mid \text{data}), P(\Delta_g < 0 \mid
\text{data})\right). $${#eq-diffexp-lfsr-recall}
...
- If $\mu_{\Delta,g} = 0$ (a measure-zero event), we can assign either sign with
error probability $0.5 = \text{lfsr}_g$.
...
$$
\text{PEFP}(S) = \left\langle \frac{\text{number of wrong-sign genes in }
S}{|S|} \;\Big|\; \text{data} \right\rangle.
...
\sum_{g \in B} \text{lfsr}_g\right). $${#eq-diffexp-pefp-monotone-proof-step1}
...
$$
\frac{1}{|B|} \sum_{g \in B} \text{lfsr}_g \geq t_1 > \frac{1}{|A|} \sum_{g' \in
A} \text{lfsr}_{g'} = 
\text{PEFP}(S(t_1)). $${#eq-diffexp-pefp-monotone-proof-step3}

The right-hand strict inequality holds because $A$ is finite and every
$\text{lfsr}_{g'} < t_1$, so the average of finitely many values each strictly
less than $t_1$ is itself strictly less than $t_1$.
...
$$
S_k = \{g : \text{lfsr}_g \leq \text{lfsr}_{(k)}\} = \{g_{(1)}, g_{(2)}, \ldots,
g_{(k)}\} $${#eq-diffexp-algorithm-step2-set} be the set of the $k$ genes with
smallest lfsr. Compute
$$
...
$$
k^* = \max\{k : \text{PEFP}_k \leq \alpha\}. $${#eq-diffexp-algorithm-step3} If
no such $k$ exists (i.e., $\text{PEFP}_1 > \alpha$), set $k^* = 0$.
...
$$
k^* = \max\{k : \text{PEFP}(S_k) \leq \alpha\} = |S(t^*)| = \max\{|S(t)| :
\text{PEFP}(S(t)) \leq \alpha\}. $${#eq-diffexp-optimality}
...
representing the minimum absolute log-fold-change deemed biologically
...
Given a threshold $\tau > 0$, we replace the probability of non-zero
differential expression with the probability of practically significant
differential expression.
...
$$
P(\Delta_g < -\tau \mid \text{data}) = \Phi\left(\frac{-\tau -
\mu_{\Delta,g}}{\sigma_{\Delta,g}}\right). $${#eq-diffexp-prob-below-tau-recall}
...
To incorporate practical significance into the lfsr framework, we define a
modified lfsr as the posterior error probability after assigning the most
probable practical-significance direction:

$$
\text{lfsr}_g(\tau) = 1 - \max\left(P(\Delta_g > \tau \mid \text{data}),
P(\Delta_g < -\tau \mid \text{data})\right). $${#eq-diffexp-lfsr-tau}

Equivalently, if we declare the direction with larger practical-significance
posterior probability, then:

$$
\text{lfsr}_g(\tau)=
\begin{cases}
P(\Delta_g \le \tau \mid \text{data}), & \text{if } P(\Delta_g > \tau \mid \text{data}) \ge P(\Delta_g < -\tau \mid \text{data}), \\
P(\Delta_g \ge -\tau \mid \text{data}), & \text{otherwise}.
\end{cases}
$${#eq-diffexp-lfsr-tau-piecewise}

This preserves the interpretation of the lfsr as a posterior probability of an
incorrect directional practical-significance call.

Note that the quantity $P(|\Delta_g| \leq \tau \mid \text{data}) = 1 -
P(\Delta_g > \tau \mid \text{data}) - P(\Delta_g < -\tau \mid \text{data})$
is a related but distinct measure: it gives the posterior probability that
the gene's differential expression magnitude falls below the practical
significance threshold. While useful, this quantity does not reduce to the
standard lfsr when $\tau = 0$ (since $P(|\Delta_g| \leq 0) = 0$ for a
continuous distribution), and therefore should not be conflated with the
modified lfsr.

**Relationship to standard lfsr.** When $\tau = 0$, the modified lfsr reduces
to the standard lfsr:

$$
\text{lfsr}_g(0) = 1 - \max\left(P(\Delta_g > 0 \mid \text{data}), P(\Delta_g <
0 \mid \text{data})\right) = \min\left(P(\Delta_g > 0 \mid \text{data}),
P(\Delta_g < 0 \mid \text{data})\right) = \text{lfsr}_g.
$${#eq-diffexp-lfsr-tau-zero}

As $\tau$ increases, both practical-significance tail probabilities decrease, so
$\max(P(\Delta_g > \tau), P(\Delta_g < -\tau))$ decreases and
$\text{lfsr}_g(\tau)$ increases. This makes the criterion more conservative and
typically reduces the number of discoveries at fixed threshold $t$.
...
\text{lfsr}_g(\tau). $${#eq-diffexp-pefp-tau}
...
$$
\frac{1}{k} \sum_{i=1}^{k} \text{lfsr}_{(i)}, $${#eq-diffexp-pefp-threshold}
