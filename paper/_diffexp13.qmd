...
## Computational Considerations {#sec-diffexp-computational}
...
genes. Below, we summarize the computational complexity of each major operation
and discuss numerical stability considerations that ensure robust performance
in practice.
...
#### Empirical vs. analytical pipeline costs

The empirical and analytical DE paths differ primarily in their dependence on
$N$ (the number of posterior samples) vs. $k$ (the low-rank dimension). The
following table summarizes the key differences:

| **Operation** | **Empirical** | **Analytical** |
|---|---|---|
| Dirichlet sampling | $O(ND)$ | Not required |
| CLR transformation | $O(ND)$ | $O(kD)$ (via centering matrix) |
| Gene-level statistics | $O(ND)$ (counting) | $O(D)$ (Gaussian CDF) |
| Pathway balance (single) | $O(ND)$ | $O(kD)$ |
| $M$ pathway balances | $O(NMD)$ | $O(kMD)$ |
| **Total for gene-level DE** | $O(ND)$ | $O(kD)$ |
| **Speedup factor** | -- | $N/k \approx 200\text{--}600\times$ |

For typical single-cell datasets with $N = 10{,}000$ posterior samples, $D =
30{,}000$ genes, and $k = 50$:

- **Empirical pipeline**: $\approx 3 \times 10^8$ operations for gene-level DE,
  completing in seconds on GPU hardware.
- **Analytical pipeline**: $\approx 1.5 \times 10^6$ operations for gene-level
  DE, completing in milliseconds.

Both pipelines produce lfsr values that feed into the same $O(D \log D)$ PEFP
threshold-finding algorithm, so the error control step is identical regardless
of which pipeline is used.

The key observation is that **all operations scale linearly or near-linearly
with the number of genes $D$**, with the exception of sorting (which is $O(D
\log D)$) and the CLR → ILR transformation (which depends on the sparsity
...
**Computational requirements** (for a single quadratic form
...
To compute tail probabilities and lfsr values accurately, we use **log-space
arithmetic**:
...
\text{for } x \ll -1, $${#eq-diffexp-log-phi-asymptotic}
...
\underline{\underline{W}}. $${#eq-diffexp-woodbury-inner-matrix}
...
**Condition number check.** Before inverting $\underline{\underline{M}}$, we can
...
$$
\underline{\underline{M}}_{\text{reg}} = \underline{\underline{M}} + \lambda
\underline{\underline{I}}_k, $${#eq-diffexp-regularization}
...
$\underline{\underline{\Sigma}}^{-1}$ explicitly; always use
@eq-diffexp-woodbury-quadratic for quadratic forms.
...
5. **Use log-space for small probabilities.** When computing lfsr values or tail
