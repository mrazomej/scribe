...
$\pi(\underline{m} \mid m_T, \underline{r})$ depends only on the concentration
parameters $\underline{r}$ and not on $p$. This is what allows the entire
...
$\text{Dirichlet}(\underline{r})$, and the shared $p$ is irrelevant for
compositional inference. Since the capture probability $\nu_c$ is gene-uniform
within each cell, the same factorization holds at the UMI level (where the
shared parameter is $\hat{p}_c = p / (\nu_c + p(1-\nu_c))$), and the
Dirichlet-Multinomial compositional term remains independent of $\nu_c$.
...
$p$ by $\theta = p / (1 - p)$ (see @eq-theta-p). The crucial property was that
when all Gamma random variables share the same rate parameter $\theta$, the
product of Gamma densities can be factored into a Gamma distribution over the
total $\Lambda = \sum_g \lambda_g$ and a Dirichlet distribution over the
proportions $\rho_g = \lambda_g / \Lambda$:
...
This factorization of the product of Gammas is what ultimately allowed $p$
...
only through $\Lambda$ and thus factors cleanly into the Gamma distribution
over $\Lambda$.

However, when $\theta_g$ vary, writing $\lambda_g = \Lambda \rho_g$ (so that
$\underline{\rho}$ lies on the simplex and $\Lambda = \sum_g \lambda_g$), the
exponent becomes
...
no longer a Dirichlet, and when the $p_g$ are gene-specific, the corresponding
$\theta_g = p_g / (1 - p_g)$ are also gene-specific, and the compositional
distribution can no longer be separated from the total.
...
$$
u_{gc} \mid r_g, p_g, \nu_c \sim \text{NB}(r_g, \hat{p}_{gc}),
\quad \hat{p}_{gc} = \frac{p_g}{\nu_c + p_g(1 - \nu_c)},
$${#eq-hier-observation-model}

for $g = 1, 2, \ldots, G$ and $c = 1, 2, \ldots, C$.
...
```{.tikz caption="Graphical model for the hierarchical gene-specific $p_g$ model. White nodes are latent variables, gray nodes are observed variables, and the deterministic node $\\hat{p}_{gc}$ (double border) is derived from $p_g$ and $\\nu_c$. The rounded boxes (plates) indicate replication: the outer plate over $p_g$, $r_g$, $\\hat{p}_{gc}$, and $u_{gc}$ is over genes $g=1,\\ldots,G$, and the inner plate over $\\nu_c$, $\\hat{p}_{gc}$, and $u_{gc}$ is over cells $c=1,\\ldots,C$." label="fig-hierarchical-p-graphical-model" additionalPackages="\\usetikzlibrary{positioning,fit,backgrounds}"}
\begin{tikzpicture}[
  >=stealth,
  ...
  inner plate/.style={draw, rounded corners, inner sep=2mm}
]
  % Hyperparameters
  \node[latent] (mu) {$\mu_p$};
  \node[latent, right=6mm of mu] (sigma) {$\sigma_p$};

  % Gene-level variables (r_g below p_g so it stays out of cell plate)
  \node[latent, below=6mm of mu, xshift=5mm] (pg) {$p_g$};
  \node[latent, below=3mm of pg] (rg) {$r_g$};

  % Cell-level capture probability
  \node[latent, right=22mm of pg] (nuc) {$\nu_c$};

  % Deterministic node (directly right of p_g, between p_g and nu_c)
  \node[det, right=5mm of pg] (phat) {$\hat{p}_{gc}$};

  % Observed count
  \node[obs, below=3mm of phat] (u) {$u_{gc}$};
  ...
  \begin{scope}[on background layer]
    \node[plate, fit=(pg)(rg)(phat)(u), label={[yshift=1mm]north east:$g=1,\ldots,G$}] (geneplate) {};
  \end{scope}

  % Cell plate (nuc, phat, u only; with r_g left of phat, fit no longer includes r_g)
  \begin{scope}[on background layer]
    \node[inner plate, fit=(nuc)(phat)(u), label={[yshift=1mm]south east:$c=1,\ldots,C$}] (cellplate) {};
  \end{scope}
\end{tikzpicture}
```

$$
\pi(\underline{r}, \underline{p}, \underline{\nu}, \mu_p, \sigma_p \mid
\underline{\underline{U}}) \propto
\left[\prod_{c=1}^C \prod_{g=1}^G
    \pi(u_{gc} \mid r_g, \hat{p}_{gc})
\right]
\left[\prod_{g=1}^G \pi(p_g \mid \mu_p, \sigma_p) \, \pi(r_g)\right]
\pi(\mu_p) \, \pi(\sigma_p) \prod_{c=1}^C \pi(\nu_c).
$${#eq-hier-full-model}
...
$\tilde{\lambda}_g$ is a latent Poisson rate draw for gene $g$, incorporating
both the gene-specific dispersion $r_g$ and the gene-specific success
probability $p_g$. Its marginal mean is $\mu_g = r_g(1-p_g)/p_g$.
...
This is the generalized composition formula. Note that at the UMI level the
scaling factor would be $(1-\hat{p}_{gc})/\hat{p}_{gc} = \nu_c(1-p_g)/p_g$.
Since $\nu_c$ is a positive constant across genes within a cell, it cancels in
the normalization step, so compositions depend only on $\underline{r}$ and
$\underline{p}$, not on $\nu_c$. This means the same Gamma-based composition
procedure works identically whether applied at the mRNA or UMI level.

To draw a compositional sample from the posterior predictive distribution under
the hierarchical model, we:
...
probabilities when it does not. Note also that compositions are the same at
both the mRNA and UMI levels, since $\nu_c$ cancels in the normalization (as
shown above). $\square$
...
$$
\text{logit}(p_g) \sim \mathcal{N}(\mu_p, \sigma_p), \quad
r_g \sim \pi(r_g), \quad
...
=
\left(
\frac{\gamma_1}{\sum_j \gamma_j},
\frac{\gamma_2}{\sum_j \gamma_j},
\ldots,
