...
$p_g = p$ at the mRNA level. This assumption was central to the beautiful
factorization of the joint distribution of independent negative binomial random
variables into the product of a negative binomial (governing total counts) and a
Dirichlet-Multinomial (governing compositions). The factorization is done at the
mRNA level (shared $p$ $\to$ common $\theta = p/(1-p)$). We learn $\underline{r}$,
$p$, and $\nu$; $\hat{p}$ is derived. The compositional term depends only on
$\underline{r}$---not on $p$ or $\hat{p}$---so normalization is justified at the
mRNA level.
...
parameters $\underline{r}$ and not on $p$ or $\hat{p}$. This is what allows the entire
...
$\theta$ comes from shared $p$ at the mRNA level: $\theta = p/(1-p)$. The
Gammas share $\theta$ because $p$ is shared, not because of $\hat{p}$. (In the
UMI space the effective parameter is $\hat{p} = p/(\nu+p(1-\nu))$, used for the
total count; the composition (Dirichlet) depends only on $\underline{r}$.) For
each gene $g$,
...
where the rate parameter $\theta = p/(1-p)$ is the same for all genes (see
@sec-dirichlet-multinomial). The crucial property was that when all Gamma random variables share the same rate
...
The compositional distribution does not use $p$ or $\theta$---it depends only on
$\underline{r}$. The factorization is possible because $\theta = p/(1-p)$ is
common (shared $p$ at the mRNA level); $\hat{p}$ does not drive the
factorization. But when each gene has its own $p_g$, the Poisson-Gamma representation becomes
...
no longer a Dirichlet, and the underlying $p$ (or the effective $\hat{p}$) can
no longer be separated from the compositional term.
...
```{.tikz caption="Graphical model for the hierarchical gene-specific $p_g$ model. White nodes are latent variables, gray nodes are observed variables, and plates indicate replication across genes and cells." label="fig-hierarchical-p-graphical-model" additionalPackages="\\usetikzlibrary{positioning,fit,backgrounds}"}
\begin{tikzpicture}[
  >=stealth,
  ...
  \begin{scope}[on background layer]
    \node[plate, fit=(pg)(rg)(u), label={[yshift=1mm]north east:$g=1,\ldots,G$}] (geneplate) {};
  \end{scope}

  % Cell plate around observations
  \begin{scope}[on background layer]
    \node[plate, fit=(u), label={[yshift=1mm]north east:$c=1,\ldots,C$}] (cellplate) {};
  \end{scope}
\end{tikzpicture}
```
