...
\begin{tikzpicture}[
  ...
  % Hyperparameters
  \node[latent] (mu) {$\mu_p$};
  \node[latent, right=of mu] (sigma) {$\sigma_p$};

  % Gene-level variables
  \node[latent, below=of mu, xshift=8mm] (pg) {$p_g$};
  \node[latent, right=18mm of pg] (rg) {$r_g$};

  % Cell-level capture probability
  \node[latent, right=18mm of rg] (nuc) {$\nu_c$};

  % Deterministic node
  \node[det, below=of rg] (phat) {$\hat{p}_{gc}$};

  % Observed count
  \node[obs, below=of phat] (u) {$u_{gc}$};
