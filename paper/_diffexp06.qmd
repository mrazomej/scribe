...
modified lfsr as the minimum of the two tail probabilities beyond $\pm \tau$:
...
This definition preserves the interpretation of the lfsr as the posterior
probability of making a sign error, now restricted to the region of practical
significance. Specifically, if we declare gene $g$ as up-regulated with
magnitude exceeding $\tau$ (because $\mu_{\Delta,g} > \tau$), then
$\text{lfsr}_g(\tau) = P(\Delta_g < -\tau \mid \text{data})$ quantifies the
posterior probability that the gene is instead down-regulated by at least
$\tau$.
...
$$
\text{lfsr}_g(0) = \min\left(P(\Delta_g > 0 \mid \text{data}), 
P(\Delta_g < 0 \mid \text{data})\right) = \text{lfsr}_g.
$${#eq-diffexp-lfsr-tau-zero}

As $\tau$ increases, $\text{lfsr}_g(\tau)$ decreases because both tail
probabilities beyond $\pm\tau$ become smaller. This makes the quantity more
stringent as evidence for practically significant, direction-specific effects.
