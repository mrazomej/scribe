...
$$
\text{PEFP}(S) = \langle \text{FDP}(S) \mid \text{data} \rangle = 
\begin{cases}
\frac{1}{|S|} \sum_{g \in S} \langle V_g \mid \text{data} \rangle & 
\text{if } |S| > 0, \\
0 & \text{if } |S| = 0.
\end{cases}
...
$$
\text{PEFP}(S) = \left\langle \frac{\text{number of wrong-sign genes in } S}{|S|} \;\Big|\; \text{data} \right\rangle.
...
$\text{lfsr}_{g'} < t_1$, so the average of finitely many values each
strictly less than $t_1$ is itself strictly less than $t_1$.
...
$$
\text{lfsr}_g(\tau) =
1 - \max\left(P(\Delta_g > \tau \mid \text{data}), 
...
$$
\text{lfsr}_g(0) =
1 - \max\left(P(\Delta_g > 0 \mid \text{data}), 
P(\Delta_g < 0 \mid \text{data})\right) =
\min\left(P(\Delta_g > 0 \mid \text{data}),
