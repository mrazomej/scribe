...
parameters $(r_g, p_g)$, where $r_g > 0$ is the dispersion and $p_g \in (0, 1)$
is the success probability. The NB probability mass function is

$$
\Pr(U = u \mid r_g, p_g) = \binom{u + r_g - 1}{u}\,
(1 - p_g)^{r_g}\, p_g^u,
\qquad u = 0, 1, 2, \ldots
$${#eq-gof-nb-pmf}
...
$$
F_{\text{NB}}(u \mid r_g, p_g) = \Pr(U \leq u \mid r_g, p_g) =
I_{1 - p_g}(r_g,\, u + 1),
$${#eq-gof-nb-cdf}
...
1. **Lower bound.** Compute $a_{gc} = F_{\text{NB}}(u_{gc} - 1 \mid r_g, p_g)$,
   with the convention that $F_{\text{NB}}(-1 \mid \cdot) = 0$ (i.e., when
   $u_{gc} = 0$, set $a_{gc} = 0$).

2. **Upper bound.** Compute $b_{gc} = F_{\text{NB}}(u_{gc} \mid r_g, p_g)$.
...
$p_g$, or the expression level $\mu_g = r_g p_g / (1 - p_g)$.
...
\Pr(U = u \mid r_{gk}, p_k),
$${#eq-gof-mixture-pmf}

where $\pi_k$ is the mixing weight for component $k$ (with $\sum_k \pi_k = 1$),
and $(r_{gk}, p_k)$ are the NB parameters for gene $g$ under component $k$.
...
$$
F_{gc}(u) = \sum_{k=1}^K \pi_k \, F_{\text{NB}}(u \mid r_{gk}, p_k).
$${#eq-gof-mixture-cdf}
...
$F_{\text{NB}}(u \mid r_{gk}, p_k)$ for any single component $k$. Using a
...
In a Bayesian workflow, the model parameters $\theta = \{r_g, p_g\}$ (or
$\theta = \{r_{gk}, p_k, \pi_k\}$ for mixtures) are not known exactly; we
...
$s_g^2$. We define the **goodness-of-fit mask** as

$$
m_g = \mathbf{1}\!\left[s_g^2 < \tau_{\text{var}}\right],
$${#eq-gof-mask-variance}

where $\tau_{\text{var}} > 1$ is a user-specified threshold. Genes with
$m_g = 1$ (residual variance within tolerance) are retained for individual DE
analysis; genes with $m_g = 0$ (excess variability not captured by the model)
are pooled into the aggregate "other" category.
...
requiring both the variance and the KS distance to be below their respective
thresholds.

#### Threshold selection

As with the Gaussianity diagnostics in @sec-diffexp-nonparametric, we advocate
a descriptive-threshold approach rather than formal hypothesis testing. A
reasonable default is:

- $\tau_{\text{var}} = 1.5$: retains genes whose residual variance is at most
  50% above the null expectation. This threshold is deliberately generous; it
  flags only genes with substantial fit problems.
...
The dominant cost is evaluating the NB CDF $F_{\text{NB}}(u \mid r, p)$ for
...
The NB CDF in @eq-gof-nb-cdf is computed by NumPyro's
`NegativeBinomialProbs.cdf()` method, which internally evaluates the regularized
incomplete beta function through JAX's `jax.lax.betainc`. The inverse normal CDF
$\Phi^{-1}$ is available as `jax.scipy.stats.norm.ppf`. Both operations are
JIT-compilable and support automatic vectorization via `jax.vmap`, enabling
efficient batch computation over the full $(C, G)$ count matrix.
