...
# Bayesian Differential Expression for Compositional Data
{#sec-diffexp}
...
This phenomenon, known as the *closure problem* [@aitchison1986], illustrates
that compositional data do not carry information about absolute quantities—only
about relative quantities. The mathematical structure of the simplex
$\Delta^{D-1}$ fundamentally differs from Euclidean space $\mathbb{R}^{D}$, and
statistical methods that ignore this distinction can produce misleading results
[@pawlowsky2015].
...
based on the negative binomial distribution [@robinson2010; @love2014], operate
on count data and model absolute transcript abundances. When applied to
normalized proportions, these methods encounter several difficulties:
...
single reference gene [@aitchison1986] produce results that depend on the
arbitrary choice of reference, complicating interpretation.
...
- The *centered log-ratio* (CLR) transformation [@aitchison1986] provides a
reference-free representation suitable for gene-level inference, where each
coordinate has a natural interpretation as a log-ratio relative to the
geometric mean.
