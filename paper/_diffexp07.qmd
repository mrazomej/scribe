...
The preceding section established that CLR-based differential expression is exactly invariant to capture probability misspecification---a structural guarantee that makes CLR the primary and most trustworthy space for compositional inference. However, CLR coordinates inherit the well-known pathologies of log-ratio transformations near the simplex boundary. Genes with very low expression occupy a region of the simplex where even tiny absolute perturbations in proportion produce dramatic CLR swings, because the logarithm amplifies small values. As a result, genes that are barely expressed can appear as strongly differentially expressed in CLR space when, in fact, their biological distributions have hardly changed.

To complement the CLR-based pipeline, we derive three metrics that quantify differential expression directly in the space of the **biological** (denoised) negative binomial distribution, completely bypassing the compositional simplex. Because the posterior samples of the NB parameters $(r_g, p_g)$ are already computed during the CLR pipeline, these metrics come at near-zero additional computational cost.

Unlike the CLR metrics, the biological-level metrics are **not** invariant to capture probability misspecification (as shown in the contrast analysis of the preceding section). Their absolute values should be interpreted cautiously when the capture model is uncertain. Nevertheless, they provide valuable complementary views: the biological log-fold change offers a non-compositional check on CLR findings, the log-variance ratio detects changes in expression noise invisible to mean-based metrics, and the Gamma Jeffreys divergence captures any distributional shift in a single scalar.
...
@sec-diffexp-empirical. The posterior mean is
...
The LFC and LVR each capture one moment of the distribution. A joint measure
asks: **how different are the two NB distributions in total?**
...
Why not compute the KL divergence between the NB distributions directly? The
NB KL divergence between $\text{NB}(r_P, p_P)$ and
$\text{NB}(r_Q, p_Q)$ is
...
1. **Screen with CLR**: use the CLR-based lfsr (with or without empirical Bayes
   shrinkage) as the primary filter, exactly as in @sec-diffexp-empirical and
   @sec-diffexp-eb.
...
4. **Flag distributional shifts**: the Jeffreys divergence is a catch-all
