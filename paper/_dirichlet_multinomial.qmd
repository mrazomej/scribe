...
\begin{tikzpicture}[
  ...
  \node[latent, below left=12mm and 6mm of p] (rg) {$r_g$};

  % Cell-level parameter
  \node[latent, below right=12mm and 6mm of p] (nuc) {$\nu_c$};

  % Deterministic node
  \node[det, below=of nuc] (phat) {$\hat{p}_c$};

  % Observed count
  \node[obs, below=12mm of rg, xshift=10mm] (u) {$u_{gc}$};
  ...
  \begin{scope}[on background layer]
    \node[plate, fit=(rg)(u), label={[yshift=1mm]north east:$g=1,\ldots,G$}] (geneplate) {};
  \end{scope}
