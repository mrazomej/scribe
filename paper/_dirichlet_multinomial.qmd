...
The generative model described above is summarized in the following graphical
model. The biophysical parameter $p$ is shared across all genes and all cells.
Each gene $g$ has its own dispersion parameter $r_g$. Each cell $c$ has its own
capture probability $\nu_c$. The effective UMI-level parameter $\hat{p}_c$ is
deterministically derived from $p$ and $\nu_c$ and is therefore not a free
parameter. The observed UMI count $u_{gc}$ depends on $r_g$ and $\hat{p}_c$.

```{.tikz caption="Graphical model for the Dirichlet-Multinomial generative model. White nodes are latent variables, gray nodes are observed variables, and the deterministic node $\\hat{p}_c$ (double border) is derived from $p$ and $\\nu_c$. The rounded boxes (plates) indicate replication: the outer plate over $r_g$ and $u_{gc}$ is over genes $g=1,\\ldots,G$, and the inner plate over $\\nu_c$, $\\hat{p}_c$, and $u_{gc}$ is over cells $c=1,\\ldots,C$." label="fig-nbdm-graphical-model" additionalPackages="\\usetikzlibrary{positioning,fit,backgrounds}"}
\begin{tikzpicture}[
  >=stealth,
  ...
  % Gene-level parameter
  \node[latent] (rg) {$r_g$};
  
  % Observed count
  \node[obs, right=5mm of rg] (u) {$u_{gc}$};

  % Deterministic node
  \node[det, right=5mm of u] (phat) {$\hat{p}_c$};

  % Cell-level parameter
  \node[latent, below=3mm of phat] (nuc) {$\nu_c$};
  
  % Global parameter
  \node[latent, above=5mm of phat] (p) {$p$};
  ...
  \begin{scope}[on background layer]
    \node[plate, fit=(rg)(u), label={[yshift=1mm]north west:$g=1,\ldots,G$}] (geneplate) {};
  \end{scope}

  % Cell plate
  \begin{scope}[on background layer]
    \node[plate, fit=(nuc)(phat)(u), label={[yshift=1mm]south east:$c=1,\ldots,C$}] (cellplate) {};
  \end{scope}
\end{tikzpicture}
```

### Factorization at the mRNA level

Since all genes are assumed to be independent and share the same parameter $p$,
the joint distribution of the true mRNA counts for a single cell,

$$
\underline{m} = \left(m_1, m_2, \ldots, m_G\right),
$${#eq-mrnavector}
...
\prod_{g=1}^G \pi(m_g \mid r_g, p),
$${#eq-mrnavector-joint}

where $\underline{r} = \left(r_1, r_2, \ldots, r_G\right)$ is the vector of
gene-specific dispersion parameters. As we will prove in detail later in this
section, the shared $p$ parameter enables a remarkable factorization: the joint
distribution of independent negative binomial random variables sharing the same
second parameter $p$ can be rewritten as

$$
\pi(\underline{m} \mid \underline{r}, p) = 
\pi(m_T \mid r_T, p)
\pi(\underline{m} \mid m_T, \underline{r}),
$${#eq-mrnavector-joint-nb}

where

$$
\pi(m_T \mid r_T, p) = \text{NB}(m_T \mid r_T, p),
$${#eq-mrnavector-joint-nb-m-dist}

$$
\pi(\underline{m} \mid m_T, \underline{r}) = 
\text{Dirichlet-Multinomial}(\underline{m} \mid m_T, \underline{r}),
$${#eq-mrnavector-joint-nb-m-dist-dirichlet-multinomial}

with $m_T$ being the total number of transcripts in the cell,

$$
m_T = \sum_{g=1}^G m_g,
$${#eq-mrnavector-joint-nb-m}
...
$$
r_T = \sum_{g=1}^G r_g.
$${#eq-mrnavector-joint-nb-r}

This factorization is the central mathematical result of this section.
Crucially, the compositional term---the Dirichlet-Multinomial---depends only on
the dispersion parameters $\underline{r}$ and **not** on $p$. This separation
is what will later enable us to use the inferred $\underline{r}$ parameters for
normalization and differential expression, independently of the value of $p$.

### Factorization at the UMI level

Since the factorization holds for any set of independent negative binomials
sharing a common second parameter, it also applies at the UMI level. For a
given cell with capture probability $\nu_c$, the effective parameter
$\hat{p}_c = p / (\nu_c + p(1 - \nu_c))$ is the same for all genes---because
both $p$ and $\nu_c$ are gene-uniform---so we can write

$$
\pi(\underline{u} \mid \underline{r}, \hat{p}_c) = 
\prod_{g=1}^G \pi(u_g \mid r_g, \hat{p}_c),
$${#eq-umivector-joint}

where $\underline{u} = (u_1, u_2, \ldots, u_G)$ is the vector of observed UMI
counts. The same factorization gives

$$
\pi(\underline{u} \mid \underline{r}, \hat{p}_c) = 
\pi(u_T \mid r_T, \hat{p}_c)
\pi(\underline{u} \mid u_T, \underline{r}),
$${#eq-umivector-joint-nb}

with $u_T = \sum_{g=1}^G u_g$. Note that $\hat{p}_c$ varies across cells
(through $\nu_c$), but the Dirichlet-Multinomial compositional term
$\pi(\underline{u} \mid u_T, \underline{r})$ is the same at both the mRNA and
UMI levels, depending only on $\underline{r}$.

## Bayesian inference

Given this generative model, we want to perform Bayesian inference on the
parameters of the model. The parameters we infer are: a single global $p$
(shared across all genes and all cells), a gene-specific dispersion $r_g$ for
each gene, and a cell-specific capture probability $\nu_c$ for each cell. The
effective parameter $\hat{p}_c = p / (\nu_c + p(1 - \nu_c))$ is derived from
$p$ and $\nu_c$, and is never directly inferred.

By Bayes' theorem, we have that

$$
\pi(\underline{r}, p, \underline{\nu} \mid \underline{\underline{U}}) \propto
\overbrace{
\pi(\underline{\underline{U}} \mid \underline{r}, p, \underline{\nu}) 
}^{\text{likelihood}}
\overbrace{\pi(\underline{r}, p, \underline{\nu})}^{\text{prior}}
$${#eq-bayes-theorem}

where $\underline{\nu} = (\nu_1, \nu_2, \ldots, \nu_C)$ is the vector of
cell-specific capture probabilities, and the proportionality constant is
independent of the model parameters.

### Prior

For the prior, we assume the parameters are independent,

$$
\pi(\underline{r}, p, \underline{\nu}) =
\prod_{g=1}^G \pi(r_g) \cdot \pi(p) \cdot \prod_{c=1}^C \pi(\nu_c).
$${#eq-prior}

The parameter $p$ is a probability, thus, $p \in [0, 1]$. The natural prior
for this interval is given by the Beta distribution. Thus, we assign

$$
p \sim \text{Beta}(\alpha_p, \beta_p), \; \alpha_p, \beta_p > 0.
$${#eq-prior-p}
...
Similarly, each $\nu_c \in [0, 1]$ receives a Beta prior,

$$
\nu_c \sim \text{Beta}(\alpha_\nu, \beta_\nu), \; \alpha_\nu, \beta_\nu > 0.
$${#eq-prior-nu}

### Likelihood

The likelihood is the product of independent negative binomial distributions
across all genes and cells. For each cell $c$ and gene $g$, the observed UMI
count $u_{gc}$ follows a negative binomial distribution with gene-specific
dispersion $r_g$ and cell-specific effective probability $\hat{p}_c$,

$$
u_{gc} \mid r_g, \hat{p}_c \sim \text{NB}(r_g, \hat{p}_c),
\quad \hat{p}_c = \frac{p}{\nu_c + p(1 - \nu_c)}.
$${#eq-likelihood-u}

Assuming all cells are independent, our Bayesian model takes the form

$$
\pi(\underline{r}, p, \underline{\nu} \mid \underline{\underline{U}}) \propto
\prod_{c=1}^C \prod_{g=1}^G
    \pi(u_{gc} \mid r_g, \hat{p}_c)
\cdot \prod_{g=1}^G \pi(r_g) \cdot \pi(p) \cdot \prod_{c=1}^C \pi(\nu_c).
$${#eq-bayes-model}

Note that inference operates on the product of independent negative binomials
directly---the NB $\times$ DM factorization derived above is **not** used during
model fitting. The factorization is instead a mathematical property of the
generative model that we exploit *after* inference: once posterior samples of
$\underline{r}$ are obtained, the mRNA-level factorization tells us that
compositions follow a $\text{Dirichlet}(\underline{r})$ distribution, which is
how we perform normalization and differential expression (see later sections).
...
parameter $p$ can be expressed as the product of a negative binomial
distribution and a Dirichlet-Multinomial distribution.
...
We can be more explicit about the structure of $u$ by substituting its
definition in terms of the $b_i$ random variables,
...
We now prove the factorization claimed in @eq-mrnavector-joint-nb. Since all
genes share the same biophysical parameter $p$, the joint distribution of true
mRNA counts $\underline{m}$ is a product of independent negative binomials with
a common second parameter. We will show that this joint distribution factors
into the product of a negative binomial (governing total counts) and a
Dirichlet-Multinomial (governing compositions). The proof works for any set of
independent negative binomials sharing a common parameter; the same result
therefore applies at the UMI level within each cell, where the shared parameter
is $\hat{p}_c$.
...
The negative binomial distribution can be expressed as a Poisson-Gamma compound
distribution. In other words, we can write
...
where $\pi(m \mid \lambda)$ is the probability mass function of a Poisson
distribution, i.e.,

$$
\pi(m \mid \lambda) = \frac{\lambda^m e^{-\lambda}}{m!},
$${#eq-poisson-dist}
...
of the gamma distribution. This parameter relates to $p$ via
...
$$
\pi(m \mid r, p) = 
\int_0^{\infty} d \lambda \, \frac{\lambda^m e^{-\lambda}}{m!}
\frac{\lambda^{r-1} e^{-\theta \lambda} \theta^r}{\Gamma(r)}.
$${#eq-nb-poisson-gamma-2}

Substituting @eq-theta-p into @eq-nb-poisson-gamma-2, results in
...
\frac{
    \lambda^{r-1} e^{-\left(\frac{p}{1-p}\right) \lambda}}
{\Gamma(r)}
\left(\frac{p}{1-p}\right)^r.
...
\left(\frac{p}{1-p}\right)^r 
\int_0^{\infty} d \lambda \,
\lambda^{m+r-1} e^{-\left(\frac{1}{1-p}\right) \lambda}.
$${#eq-nb-poisson-gamma-4}

Let's focus on the integral. Let us define

$$
x = \left(\frac{1}{1-p}\right) \lambda.
$${#eq-nb-poisson-gamma-5}

Then, we have that

$$
dx = \left(\frac{1}{1-p}\right) d\lambda.
...
\int_0^{\infty} d \lambda \,
\lambda^{m+r-1} e^{-\left(\frac{1}{1-p}\right) \lambda} = 
\int_0^{\infty} dx \, (1-p) [(1-p) x]^{m+r-1} e^{-x}.
...
original expression for $\pi(m \mid r, p)$,
...
$$
\pi(m \mid r, p) = \frac{\Gamma(m+r)}{m!\Gamma(r)}(1-p)^m \, p^r,
...
$$
\pi(m \mid r, p) = \frac{(m+r-1)!}{(r-1)!m!}(1-p)^m p^r,
...
$$
\pi(m \mid r, p) = \binom{m+r-1}{m}(1-p)^m p^r.
...
$$
\pi(\underline{m} \mid \underline{r}, p) =
\prod_{g=1}^G \pi\left(m_g \mid r_g, p\right),
$${#eq-nb-joint-dist}

can be expressed as

$$
\pi(\underline{m} \mid \underline{r}, p) = \prod_{g=1}^G 
\int_0^{\infty} d \lambda_g \, 
\overbrace{
\pi\left(m_g \mid \lambda_g\right) 
}^{\text{Poisson}}
\times
\underbrace{
...
\overbrace{
\prod_{g=1}^G \pi\left(m_g \mid \lambda_g \right) 
...
$$
\pi(\underline{m} \mid \underline{\lambda}) =
\prod_{g=1}^G
\pi\left(m_g \mid \lambda_g\right) =
\prod_{g=1}^G \frac{\lambda_g^{m_g}}{m_{g}!} e^{-\lambda_g}.
$${#eq-joint-poisson}

Next, we define the sum of the Poisson random variables as $m_T = \sum_g m_g$.
The claim is that if we write the product of a single Poisson and a multinomial,

$$
\pi(\underline{m} \mid m_T, \underline{\lambda}) 
\pi(m_T \mid \underline{\lambda}) =
\left[
    \left(m_T! \prod_{g=1}^G \frac{\rho_g^{m_g}}{m_g!} \right)
\left(
    \frac{\left(\sum_{j=1}^G \lambda_j\right)^{m_T}}{m_T!} 
...
\prod_{g=1}^G
\left[
    \frac{1}{m_{g}!}\left(\frac{\lambda_g}{\sum_{j=1}^G \lambda_j}\right)^{m_g}
\right]
\left(\sum_{j=1}^G \lambda_j\right)^{m_T} e^{-\sum_{g=1}^G \lambda_g}.
...
\pi(\underline{m} \mid m_T, \underline{\lambda}) 
\pi(m_T \mid \underline{\lambda}) =
\frac{1}{\left(\sum_{g=1}^G \lambda_g\right)^{m_T}} 
\prod_{g=1}^G 
\frac{\lambda_g^{m_g}}{m_{g}!}\left(\sum_{g=1}^G \lambda_g\right)^{m_T} 
...
In the same way we defined $m_T$ as the sum of the Poisson random variables, we
...
independent negative binomial random variables with a shared $p$ parameter,
...
\lambda_g$ and a multinomial distribution with parameters $m_T$ and
$\underline{\rho}$
...
\underbrace{
\pi(\underline{m} \mid m_T, \underline{\rho})
}_{\text{multinomial}}
\times
\overbrace{
\pi(m_T \mid \Lambda)
...
$$
\int_0^{\infty} d \Lambda \,
\pi(m_T \mid \Lambda) \times \pi\left(\Lambda \mid r_T, \theta\right) =
\operatorname{NB}\left(m_T \mid r_T, p\right).
...
the probability of observing counts $\underline{m}$ given $m_T$ trials and
success probabilities $\underline{\rho}$, is

$$
\pi(\underline{m} \mid m_T, \underline{\rho}) = 
\frac{m_T!}{\prod_{g=1}^G m_g!} \prod_{g=1}^G \rho_g^{m_g}.
$${#eq-multinomial-pmf}
...
\right) 
\left( 
    \frac{m_T!}{\prod_{k=1}^G m_k!} \prod_{g=1}^G \rho_g^{m_g} 
\right).
...
\frac{
    ...
    \left(\prod_{k=1}^G \Gamma(r_k)\right) \left(\prod_{k=1}^G m_k!\right)
} 
\int d^{G-1} \underline{\rho} \, 
\left( 
    \prod_{g=1}^G \rho_g^{r_g-1} \right) \left( \prod_{g=1}^G \rho_g^{m_g} 
\right).
...
math. First, we define $\alpha_g = m_g + r_g$ to be our parameter for the
...
the parameters results in $\sum_{g=1}^G (m_g + r_g) = \sum_{g=1}^G m_g +
\sum_{g=1}^G r_g = m_T + r_T$. Applying @eq-dirichlet-integral-value, the
integral in @eq-dm-integral-rearranged-2 evaluates to

$$
\int d^{G-1} \underline{\rho} \, \prod_{g=1}^G \rho_g^{m_g + r_g - 1} = 
\frac{\prod_{g=1}^G \Gamma(m_g + r_g)}{\Gamma(m_T + r_T)}.
...
$m_T! = \Gamma(m_T+1)$ and $m_k! = \Gamma(m_k+1)$. Substituting these into
@eq-dm-intermediate-result, we get

$$
\pi(\underline{m} \mid m_T, \underline{r}) = 
\frac{
    \Gamma(r_T) \Gamma(m_T+1)
}{
    \left(
        \prod_{k=1}^G \Gamma(r_k)\right) \left(\prod_{k=1}^G \Gamma(m_k+1)
    \right)
} \times
\frac{\prod_{g=1}^G \Gamma(m_g + r_g)}{\Gamma(m_T + r_T)}.
...
\pi(\underline{m} \mid m_T, \underline{r}) =
\frac{
    \Gamma\left(r_T\right) \Gamma(m_T+1)
}{
    \Gamma\left(m_T+r_T\right)
} 
\prod_{g=1}^G \frac{
    \Gamma\left(m_g+r_g\right)
}{
    \Gamma\left(r_g\right) \Gamma\left(m_g+1\right)
}.
...
a shared $p$ parameter is equivalent to the product of a negative binomial
distribution and a Dirichlet-Multinomial distribution. This has several
important consequences for our model for single-cell RNA-seq data:

First, the factorization is enabled by the assumption that the biophysical
parameter $p$ is shared across all genes **and** all cells. This is an
mRNA-level assumption about the physics of gene expression. Since the capture
probability $\nu_c$ is also gene-uniform within each cell, the effective
UMI-level parameter $\hat{p}_c = p / (\nu_c + p(1 - \nu_c))$ is likewise shared
across genes within each cell (though it varies from cell to cell). The
factorization therefore holds at both the mRNA and UMI levels.

Second, it is important to note that inference does **not** use the NB $\times$
DM factorization. The codebase fits the product of independent negative
binomials $\prod_g \text{NB}(u_{gc} \mid r_g, \hat{p}_c)$ directly, inferring
the global $p$, gene-specific $\underline{r}$, and cell-specific
$\underline{\nu}$. The factorization is instead a mathematical property of the
generative model that we exploit *after* inference: once posterior samples of
$\underline{r}$ are obtained, the factorization tells us that compositions
follow a $\text{Dirichlet}(\underline{r})$ distribution. This is how we perform
normalization, differential expression, and denoising, purely from the
$\underline{r}$ parameters.

Third, in the construction of the negative binomial/Dirichlet multinomial model,
we summoned the use of $\underline{\rho}$ parameters that lie on the unit
