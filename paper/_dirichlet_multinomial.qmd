...
$$
M_{\Lambda}(t) = \left\langle\exp \left(t \sum_{g=1}^G \lambda_g\right)\right\rangle_{\pi(\underline{\lambda})}
$${#eq-mgf-lambda-2}

Using the property of exponentials, we can rewrite this as a product

$$
M_{\Lambda}(t) = \left\langle e^{t \lambda_1} e^{t \lambda_2} \cdots e^{t \lambda_G}\right\rangle_{\pi(\underline{\lambda})}
$${#eq-mgf-lambda-3}

By independence of the $\lambda_g$ terms, we can factor the expectation

$$
M_{\Lambda}(t) = \left\langle e^{t \lambda_1}\right\rangle_{\pi\left(\lambda_1\right)}\left\langle e^{t \lambda_2}\right\rangle_{\pi\left(\lambda_2\right)} \cdots\left\langle e^{t \lambda_G}\right\rangle_{\pi\left(\lambda_G\right)}
$${#eq-mgf-lambda-4}

Finally, we recognize that each expectation is the MGF of an individual $\lambda_g$
...
Substituting the MGF form for each $\lambda_g$
...
$r_T = \sum_{g=1}^G r_g$ and $\theta$. Therefore, we have shown the desired
property of @eq-sum-gamma-dist.
...
$\underline{r}$. To show show this, we transform the dist. of all
...
$\lambda_g$ parameters in terms of the $\rho_g$ parameters

$$
\lambda_g = \Lambda \rho_g \; \text{for} \; g \in \{1, 2, \ldots, G-1\},
$${#eq-lambda-rho-1}
...
The Jacobian matrix $\underline{\underline{J}}$ for this transformation from
$\underline{\lambda} = (\lambda_1, \ldots, \lambda_G)$ to $\underline{V}$ has
entries $J_{ab} = \frac{\partial \lambda_a}{\partial V_b}$. Here, $a \in \{1,
\ldots, G\}$ indexes the original variables $\lambda_a$, and $V_b$ refers to the
$b$-th variable in the vector $\underline{V}$ (i.e., $V_1 = \Lambda, V_2 =
\rho_1, \ldots, V_G = \rho_{G-1}$).
...
Jacobian matrix, let's call it $\underline{\underline{J}}'$, now takes the form
...
Since the last row is $(1, 0, \ldots, 0)$, the expansion simplifies
significantly

$$
\det(\underline{\underline{J}}) = 1 \cdot C_{G1} + \sum_{j=2}^{G} (0 \cdot C_{Gj}) = C_{G1},
...
$$
\frac{1}{B(\underline{r})} \equiv \frac{\Gamma\left(r_T\right)}{\prod_{g=1}^G \Gamma\left(r_g\right)} .
$${#eq-dirichlet-normalization}

Notice that the denominator for this normalization constant is the same as the
denominator in @eq-jacobian-9. Thus, multiplying @eq-jacobian-8 by $\Gamma(r_T)
/ \Gamma(r_T)$ suffices to turn both terms into the desired distributions. This
then shows our original claim that the joint distribution of $\underline{V}$ is
the product of a Gamma distribution and a Dirichlet distribution

$$
\pi\left(\underline{V} \mid \underline{r}, \theta\right) =
\operatorname{Gamma}(\Lambda \mid r, \theta) \times 
...
previous section. Recall that we are after the joint distribution of independent
negative binomial random variables with a shared $\hat{p}$ parameter,
...
$$
\int_0^{\infty} d \Lambda 
\pi(u_T \mid \Lambda) \pi\left(\Lambda \mid r_T, \theta\right) =
...
Let $r_T = \sum_{k=1}^G r_k$. Then the Dirichlet PDF can be written as
...
The integral term is related to the normalization constant of a Dirichlet
distribution. Specifically, for a Dirichlet distribution with parameters
$\underline{\alpha} = (\alpha_1, \ldots, \alpha_G)$, its PDF is

$$
\text{Dir}(\underline{\rho} \mid \underline{\alpha}) = 
\frac{
    \Gamma\left(\sum_{k=1}^G \alpha_k\right)
}{
    \prod_{k=1}^G \Gamma(\alpha_k)
} 
\prod_{g=1}^G \rho_g^{\alpha_g-1}.
$${#eq-dirichlet-general-pdf}

Since the Dirichlet distribution must integrate to 1 over the simplex $S = \{
\underline{\rho} : \rho_g \ge 0, \sum \rho_g = 1 \}$, we have
...
\prod_{g=1}^G \rho_g^{\alpha_g-1} = 1.
$${#eq-dirichlet-integral-one}

From this, we can deduce the value of the integral part of
@eq-dm-integral-rearranged-2
...
In our specific case, the parameters for the integral in
@eq-dm-integral-rearranged-2 are $\alpha_g = u_g + r_g$. Therefore, the sum of
these parameters is $\sum_{g=1}^G (u_g + r_g) = \sum_{g=1}^G u_g + \sum_{g=1}^G
r_g = u_T + r_T$. Applying @eq-dirichlet-integral-value, the integral in
@eq-dm-integral-rearranged-2 evaluates to
...
for a parametric model of the counts. Moreover, this functional form already
accounts for the conversion from mRNA molecules to our observed UMI counts. Our
derivation shows that with the relatively benign assumption that the
$\hat{p}$ parameter is shared across all genes, the resulting joint distribution
has a much simpler functional form.
...
occupied by each of the $G$ genes. Usually, when we want to "normalize" a 
dataset, what we want is the equivalent of estimating these parameters. However,
in our model, we marginalized over their values to find the joint distribution.