...
transcriptional profile of a cell. This normalization is defined at the mRNA
level (the composition of the transcriptome); the derivation in UMI space
follows from this assumption plus the mRNA-level capture process. Moreover, this
assumption is not as strong as it may seem, since the negative binomial
distribution is degenerate on its parameters. In other words, different
combinations of $r_g$ and $p_g$ can lead to distributions of very similar shape,
making the choice of a single $p_g$ not as critical as it may seem.
...
assumed that all genes share the same parameter $p$ at the mRNA level. After
capture, the effective parameter in the UMI model is $\hat{p} = p/(\nu+p(1-\nu))$;
$\hat{p}$ is shared across genes within a cell and may be cell-specific if $\nu$
varies by cell.
...
parameter $p$ (at the mRNA level; the factorization is done at the mRNA level
with $\theta = p/(1-p)$. We learn $r$, $p$, and $\nu$; $\hat{p}$ is derived.
Only then is the factorization possible.) can be expressed as the product of a
negative binomial distribution and a Dirichlet-Multinomial distribution.
...
$r$ and $\hat{p}$. We assume that $p$ is shared at the mRNA level. The
factorization (total NB $\times$ DM) is done at the mRNA level; we learn $r$, $p$,
and $\nu$, and $\hat{p} = p/(\nu+p(1-\nu))$ is derived. In the UMI model the joint
has shared $\hat{p}$ (and the same compositional DM($\underline{r}$)); the common
Gamma rate $\theta$ in the derivation comes from shared $p$ ($\theta = p/(1-p)$
at the mRNA level). (If $\nu$ is cell-specific, $\hat{p}$ is cell-specific.)
Earlier in
...
The factorization (product of Gammas $\to$ Gamma($\Lambda$) $\times$ Dirichlet($\underline{\rho}$))
is done at the mRNA level. At the mRNA level, the success probability $p$ is
shared across genes, so the Gamma rate $\theta = p/(1-p)$ is common. We learn the
parameters $r$, $p$, and $\nu$; the effective UMI parameter is
$\hat{p} = p/(\nu+p(1-\nu))$. Only then is the factorization possible. Below we
give the compound representation; in the UMI space the marginal is
$\text{NB}(r, \hat{p})$ and the same compositional structure (DM($\underline{r}$))
holds.
...
of the gamma distribution. This parameter relates to the (mRNA-level) success
probability $p$ by $\theta = p/(1-p)$; the Gammas share $\theta$ because $p$ is
shared at the mRNA level. (In the UMI space the compound representation uses the
effective parameter $\hat{p} = p/(\nu+p(1-\nu))$; the rate is
$\theta = \hat{p}/(1-\hat{p})$ and the marginal is $\text{NB}(r, \hat{p})$.)
...
$\underline{r}$. Thus the compositional (normalization) distribution does not
depend on $p$ or $\hat{p}$---only on $\underline{r}$. To show this, we transform the distribution of all
...
independent negative binomials with shared $p$ parameter (at the mRNA level) is
equivalent to the product of a negative binomial distribution and a
dirichlet-multinomial distribution (not to be confused with the product of a
Dirichlet distribution and a multinomial distribution). The factorization is at
the mRNA level (shared $p$ $\to$ common $\theta = p/(1-p)$). We learn $r$, $p$,
and $\nu$; $\hat{p}$ is derived. The UMI likelihood has the same structure (total
$\text{NB}(r_T, \hat{p})$, composition DM($\underline{r}$)); the compositional
part depends only on $\underline{r}$.
...
The factorization happens at the mRNA level under shared $p$ ($\theta = p/(1-p)$).
We learn $r$, $p$, and $\nu$; $\hat{p} = p/(\nu+p(1-\nu))$ is derived and used for
the UMI total. Only then is the factorization possible. Inferring $p$ and $\nu$
(hence $\hat{p}$) lets us deconvolve experimental noise from biology. The
compositional part depends only on $\underline{r}$.

As we just showed, the joint distribution of independent negative binomials with
shared $\hat{p}$ parameter (which follows from shared $p$ and $\nu$) is
equivalent to the product of a negative binomial distribution and a
Dirichlet-Multinomial distribution. This has two important
...
The shared-$p$ assumption at the mRNA level yields the factorization above;
the resulting joint distribution has a much simpler functional form---the product
of two distributions rather than the product of $G$ distributions.
...
occupied by each of the $G$ genes. The target of normalization is the underlying
(mRNA) transcriptome composition; the Dirichlet over $\underline{\rho}$ is
justified by the shared-$p$ assumption. Commonly, when we want to "normalize" a
...
parameters (not a single point estimate) to account for overdispersion. If desired, we can still report the mean of this distribution to
