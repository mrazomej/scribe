...
## Analytical Gene-Level Differential Expression {#sec-diffexp-analytical-gene}

When the Gaussianity diagnostics developed in @sec-diffexp-gaussianity indicate
that the posterior CLR marginals are well-approximated by Gaussians, the
difference distribution between two conditions is itself Gaussian with
closed-form parameters. This enables analytical computation of all gene-level
statistics---posterior means, standard deviations, tail probabilities, and local
false sign rates---at $O(kD)$ cost rather than $O(ND)$. The analytical lfsr
values produced here feed into the same PEFP error control machinery
(@sec-diffexp-pefp) and are fully compatible with the empirical lfsr from
@sec-diffexp-empirical, enabling a hybrid workflow where each gene is analyzed
by the most appropriate method.

With the machinery for transforming Gaussian distributions between coordinate
systems in place, we derive the posterior distribution of gene-level differences
and show how to compute key statistics—posterior means, standard deviations,
tail probabilities, and the local false sign rate.
...
\underline{z}_{B,\text{CLR}}. $${#eq-diffexp-delta-definition}
...
$$
\langle \underline{\Delta} \rangle = \langle \underline{z}_{A,\text{CLR}} \rangle - 
\langle \underline{z}_{B,\text{CLR}} \rangle = 
...
$$
\text{Cov}(\underline{\Delta}) = \text{Cov}(\underline{z}_{A,\text{CLR}} -
\underline{z}_{B,\text{CLR}}) = \text{Cov}(\underline{z}_{A,\text{CLR}}) +
\text{Cov}(\underline{z}_{B,\text{CLR}}). $${#eq-diffexp-delta-cov-step1}
...
\langle (\underline{X} - \underline{Y})(\underline{X} - \underline{Y})^{\top} \rangle - 
\langle \underline{X} - \underline{Y} \rangle\langle \underline{X} - \underline{Y} \rangle^{\top}.
$${#eq-diffexp-cov-expansion}

Expanding the first term:

$$
\langle (\underline{X} - \underline{Y})(\underline{X} - \underline{Y})^{\top}
\rangle = \langle \underline{X}\underline{X}^{\top} \rangle - \langle
\underline{X}\underline{Y}^{\top} \rangle - \langle
\underline{Y}\underline{X}^{\top} \rangle + \langle
\underline{Y}\underline{Y}^{\top} \rangle. $${#eq-diffexp-cov-expansion-step2}

By independence, $\langle \underline{X}\underline{Y}^{\top} \rangle = \langle
\underline{X} \rangle\langle \underline{Y} \rangle^{\top}$ and $\langle
\underline{Y}\underline{X}^{\top} \rangle = \langle \underline{Y} \rangle\langle
\underline{X} \rangle^{\top}$. Therefore,

$$
\text{Cov}(\underline{X} - \underline{Y}) = 
\left(\langle \underline{X}\underline{X}^{\top} \rangle - 
\langle \underline{X} \rangle\langle \underline{X} \rangle^{\top}\right) + 
\left(\langle \underline{Y}\underline{Y}^{\top} \rangle - 
\langle \underline{Y} \rangle\langle \underline{Y} \rangle^{\top}\right) = 
...
\underline{\underline{\Sigma}}_{B,\text{CLR}}. $${#eq-diffexp-delta-cov}
...
\text{diag}(\underline{d}_{A,\text{CLR}}), $${#eq-diffexp-sigma-a-structure}
...
We can express this in low-rank plus diagonal form by concatenating the low-rank
factors:
...
\underline{d}_{B,\text{CLR}}. $${#eq-diffexp-d-delta}
...
$$
\sigma_{\Delta,g}^2 = [\underline{\underline{\Sigma}}_{\Delta}]_{gg} =
...
$$
\sigma_{\Delta,g} = \sqrt{\sigma_{\Delta,g}^2}. $${#eq-diffexp-sd-delta-g}
...
$$
P(\Delta_g > 0 \mid \text{data}) = P\left(\mathcal{N}(\mu_{\Delta,g},
\sigma_{\Delta,g}^2) > 0\right) = 1 -
...
\mu_{\Delta,g}}{\sigma_{\Delta,g}}\right). $${#eq-diffexp-prob-below-tau}
...
$$
\text{lfsr}_g = \min\left(\Phi(z_g), 1 - \Phi(z_g)\right) = \min\left(\Phi(z_g),
\Phi(-z_g)\right). $${#eq-diffexp-lfsr-formula}
...
$$
\Phi(-|z_g|) \approx \frac{1}{|z_g|\sqrt{2\pi}}
\exp\left(-\frac{z_g^2}{2}\right),
...
$$
p_g \approx 2 \cdot \text{lfsr}_g. $${#eq-diffexp-pvalue-lfsr-relation}

Thus, the lfsr and p-value convey similar information about the strength of
evidence against the null hypothesis, but the lfsr has the advantage of a direct
