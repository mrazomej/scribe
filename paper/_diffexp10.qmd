...
ultimately wish to estimate---and $s_g$ is the known standard error. The
Gaussian assumption for the observation model is justified by the central limit
theorem: $\hat{\Delta}_g$ is the average of $N$ posterior samples, and for
typical sample sizes $N \geq 1{,}000$, the sampling distribution of the mean
is well-approximated by a Gaussian regardless of the shape of the individual
$\Delta_g^{(s)}$ samples. This is precisely the regime where the CLT is most
reliable.
