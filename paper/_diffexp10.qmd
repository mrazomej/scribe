...
## Transforming Gaussian Parameters Between Coordinate Systems {#sec-diffexp-gaussian-transforms}

This section develops the mathematical machinery for the parametric differential
expression path. When the Gaussianity diagnostics from the preceding section
confirm that the posterior CLR marginals are well-approximated by Gaussians, the
low-rank logistic-normal model fitted in @sec-normalization can be transformed
analytically between coordinate systems. The formulas derived here---for mean
vectors, covariance matrices, and their low-rank components---enable the
closed-form gene-level and gene-set inference developed in the sections that
follow.
...
\text{diag}(\underline{d}_{\text{ALR}}). $${#eq-diffexp-alr-covariance}

For differential expression analysis, we require Gaussian distributions in CLR
and ILR spaces. We derive explicit formulas for transforming
...
\underline{\mu}_{\text{full}}. $${#eq-diffexp-mean-clr}
...
\underline{0}^{\top} & 0 \end{bmatrix} \in \mathbb{R}^{D \times D}.
...
$$
\underline{\underline{\Sigma}}_{\text{CLR}} = \underline{\underline{C}}
...
$$
\underline{\underline{W}}_{\text{CLR}} = \underline{\underline{C}}
\underline{\underline{W}}_{\text{full}}. $${#eq-diffexp-w-clr}
...
\underline{0}^{\top} & 0 \end{bmatrix}, $${#eq-diffexp-w-full-product}
...
$$
\underline{d}_{\text{full}} = \begin{bmatrix} \underline{d}_{\text{ALR}} \\
0 \end{bmatrix} \in \mathbb{R}^{D}. $${#eq-diffexp-d-embed}
...
$$
\underline{d}_{\text{CLR}} \stackrel{?}{=} \underline{\underline{C}}
\underline{d}_{\text{full}}. $${#eq-diffexp-d-naive}

This is **incorrect** because the diagonal of a matrix product
$\underline{\underline{C}} \text{diag}(\underline{d}_{\text{full}})
\underline{\underline{C}}$ is not generally equal to $\underline{\underline{C}}
\underline{d}_{\text{full}}$. The centering matrix introduces off-diagonal terms
that contribute to the diagonal of the transformed covariance.
...
$$
\left[\underline{\underline{C}} \text{diag}(\underline{d}_{\text{full}})
\underline{\underline{C}}\right]_{gg} = \sum_{j=1}^{D} C_{gj}^2
d_{\text{full},j}. $${#eq-diffexp-d-exact-step2}
...
d_{\text{full},g}\right). $${#eq-diffexp-d-exact-step6}
...
\underline{\mu}_{\text{CLR}}. $${#eq-diffexp-mean-ilr}
...
\underline{\underline{W}}_{\text{ILR}}^{\top} = \underline{\underline{V}}
\underline{\underline{W}}_{\text{CLR}}
...
The diagonal component of the ILR covariance cannot be computed as simply as the
CLR diagonal because $\underline{\underline{V}}$ is not a diagonal matrix.
...
diagonal entries, with weights given by the squared entries of the $i$-th row of
$\underline{\underline{V}}$. Since each row of $\underline{\underline{V}}$ is
sparse (for the Helmert basis), this computation can be performed efficiently.
...
Moreover, for applications requiring only quadratic forms $\underline{v}^{\top}
\underline{\underline{\Sigma}}^{-1} \underline{v}$ rather than the full inverse
...
$$
\log \det(\underline{\underline{\Sigma}}) = \sum_{i=1}^{D} \log d_i + \log
\det\left(\underline{\underline{I}}_k + \underline{\underline{W}}^{\top}
\text{diag}(\underline{d})^{-1} \underline{\underline{W}}\right).
...
The Woodbury identity and matrix determinant lemma ensure that all operations on
low-rank plus diagonal covariance matrices have complexity $O(k^2 D)$ rather
than $O(D^3)$:
