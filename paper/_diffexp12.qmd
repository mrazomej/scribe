...
## Analytical Gene-Set and Pathway Analysis {#sec-diffexp-analytical-pathway}

This section develops the parametric counterpart to the empirical pathway
enrichment framework of @sec-diffexp-empirical-pathway. When the Gaussian
approximation is valid, linear contrasts in CLR space and balances in ILR space
have closed-form posterior distributions, enabling exact computation of
pathway-level statistics at $O(kD)$ cost per pathway rather than the $O(ND)$
cost of the empirical approach. For each pathway, the parametric and empirical
approaches estimate the same posterior balance; the parametric path trades the
Gaussian distributional assumption for a computational speedup of $N/k$
(typically $\approx 200$--$600\times$).
...
$$
\sum_{g=1}^{D} c_g = 0. $${#eq-diffexp-contrast-constraint}
...
contrast is not trivial (all zeros). This can occur if $\underline{c}' = \alpha
\underline{1}$ for some scalar $\alpha$. A simple check is
...
$$
\sigma_{\gamma}^2 = \underline{c}^{\top} \underline{\underline{\Sigma}}_{\Delta}
\underline{c}. $${#eq-diffexp-contrast-variance}
...
\underline{\underline{W}}_{\Delta}^{\top} \underline{c} + \underline{c}^{\top}
\text{diag}(\underline{d}_{\Delta}) \underline{c}.
...
$$
\sigma_{\gamma}^2 = \sum_{j=1}^{k_A+k_B} w_j^2 + \sum_{g=1}^{D} c_g^2
d_{\Delta,g}. $${#eq-diffexp-contrast-variance-final}
...
$$
b(S_+, S_-) = \frac{1}{|S_+|} \sum_{g \in S_+} \log(\rho_g) - \frac{1}{|S_-|}
\sum_{g \in S_-} \log(\rho_g).
...
$$
\underline{z}_{\text{ILR}} = \underline{\underline{V}}
\underline{z}_{\text{CLR}}, $${#eq-diffexp-ilr-recall}

where $\underline{\underline{V}}$ is an orthonormal matrix satisfying
@eq-diffexp-helmert-properties.

Each row of $\underline{\underline{V}}$ defines a pre-specified balance. For the
Helmert basis @eq-diffexp-helmert-formula, the $i$-th row represents the balance
between gene $i+1$ and the first $i$ genes. More generally, we can construct
custom ILR bases where each row corresponds to a biologically meaningful
balance.
...
\sigma_{\Delta,\text{ILR},i}^2), $${#eq-diffexp-delta-ilr-marginal}
...
$$
\sigma_{\Delta,\text{ILR},i}^2 = [\underline{\underline{V}}
\underline{\underline{\Sigma}}_{\Delta} \underline{\underline{V}}^{\top}]_{ii} =
...
condition B." This provides a more detailed view of differential expression
than simply listing individual genes.
...
$$
c_{m,g} = \begin{cases} \frac{1}{|P_m|} & \text{if } g \in P_m, \\
-\frac{1}{D - |P_m|} & \text{if } g \notin P_m.
\end{cases} $${#eq-diffexp-pathway-contrast-vector}
