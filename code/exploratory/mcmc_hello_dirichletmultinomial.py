...
def model(obs=None, num_dimensions=3):
    """
    Model for multivariate data following the Negative Binomial-Dirichlet Multinomial (NBDM) approach.
    
    This model assumes that for each sample:
        1. The total count follows a Negative Binomial distribution with parameters r_total and p
        2. Given the total count, the individual dimension counts follow a Dirichlet-Multinomial
           distribution with concentration parameters r
    ...
    # Sample r parameters for each dimension from Gamma priors
    ...
    if obs is not None:
        with numpyro.plate("observations", obs.shape[0]):
            # Get total counts for each observation
            total_counts = jnp.sum(obs, axis=1)
            
            # Likelihood for the total counts - one for each observation
            numpyro.sample(
                "total_counts",
            ...
            # Likelihood for the individual counts - one for each observation
            numpyro.sample(
                "obs",
        ...
        with numpyro.plate("observations", 1):
            # Sample total counts from Negative Binomial
            total_counts = numpyro.sample("total_counts", dist.NegativeBinomialProbs(r_total, p))
            
            # Sample individual counts from Dirichlet-Multinomial
            numpyro.sample("obs", dist.DirichletMultinomial(r, total_count=total_counts))
