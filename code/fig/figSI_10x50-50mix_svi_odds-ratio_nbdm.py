...
# Set title
ax.set_title(f"SVI loss for NBDM model\n with odds-ratio parameterization")

# Save figure
fig.savefig(
    f"{FIG_DIR}/figSI_10x50-50mix_svi_{parameterization.replace('_', '-')}_"
    f"{model_type}_loss.png",
    bbox_inches="tight"
)
fig.savefig(
    f"{FIG_DIR}/figSI_10x50-50mix_svi_{parameterization.replace('_', '-')}_"
    f"{model_type}_loss.pdf",
    bbox_inches="tight"
)
...
print("Plotting posteiror distributions...")
...
scribe.viz.plot_posterior(
    ax=axes[0],
    distribution=results_subset.get_distributions(backend="scipy")["phi"],
...
# Define colors palette as magma with n_genes colors
col = sns.color_palette("magma", n_colors=n_genes)
...
for i in range(n_genes):
    ...
    scribe.viz.plot_posterior(
        ax=axes[1],
        distribution=results_subset.get_distributions(
            backend="scipy",
            split=True
        )["mu"][i],
        color=col[i],
...
# Set title
axes[1].set_title(r"$q_\phi(\mu_1, \ldots, \mu_G)$")
...
fig.savefig(
    f"{FIG_DIR}/figSI_10x50-50mix_svi_{parameterization.replace('_', '-')}_"
    f"{model_type}_posterior.png",
    bbox_inches="tight"
)
fig.savefig(
    f"{FIG_DIR}/figSI_10x50-50mix_svi_{parameterization.replace('_', '-')}_"
    f"{model_type}_posterior.pdf",
    bbox_inches="tight"
)
...
results_subset.get_ppc_samples(n_samples=1_000)
...
svi_results.get_ppc_samples(n_samples=1)
# %% ---------------------------------------------------------------------------
...
umap_ppc = umap.UMAP(n_components=2).fit_transform(
    svi_results.predictive_samples[0, :, :]
)
...
fig.savefig(
    f"{FIG_DIR}/figSI_10x50-50mix_svi_{parameterization.replace('_', '-')}_"
    f"{model_type}_umap.png",
    bbox_inches="tight"
)
fig.savefig(
    f"{FIG_DIR}/figSI_10x50-50mix_svi_{parameterization.replace('_', '-')}_"
    f"{model_type}_umap.pdf",
    bbox_inches="tight"
)
