...
OUTPUT_DIR = f"{scribe.utils.git_root()}/output/singer/nbvcp"
...
scribe.viz.plot_posterior(
    ax=axes[0],
    distribution=svi_results.get_distributions(backend="scipy")["phi"],
...
for i in range(n_genes):
    ...
    scribe.viz.plot_posterior(
        ax=axes[1],
        distribution=svi_results.get_distributions(
            backend="scipy",
            split=True
        )["mu"][i],
        color=col[i],
...
fig.savefig(
    f"{FIG_DIR}/figSI_singer_svi_{parameterization.replace('_', '-')}_"
    f"{model_type}_pcapture.png",
    bbox_inches="tight"
)
fig.savefig(
    f"{FIG_DIR}/figSI_singer_svi_{parameterization.replace('_', '-')}_"
    f"{model_type}_pcapture.pdf",
    bbox_inches="tight"
)
