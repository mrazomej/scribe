...
# Set title
ax.set_title(
    f"SVI loss for NBVCP-mixture model\n with odds-ratio parameterization"
)

# Save figure
fig.savefig(
    f"{FIG_DIR}/figSI_10x50-50mix_svi_"
    f"{parameterization.replace('_', '-')}_"
    f"{model_type.replace('_', '-')}-split_loss.png",
    bbox_inches="tight"
)
fig.savefig(
    f"{FIG_DIR}/figSI_10x50-50mix_svi_"
    f"{parameterization.replace('_', '-')}_"
    f"{model_type.replace('_', '-')}-split_loss.pdf",
    bbox_inches="tight"
)
...
fig.savefig(
    f"{FIG_DIR}/figSI_10x50-50mix_svi_{parameterization.replace('_', '-')}_"
    f"{model_type.replace('_', '-')}-split_umap.png",
    bbox_inches="tight"
)
fig.savefig(
    f"{FIG_DIR}/figSI_10x50-50mix_svi_{parameterization.replace('_', '-')}_"
    f"{model_type.replace('_', '-')}-split_umap.pdf",
    bbox_inches="tight"
)
