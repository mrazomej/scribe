...
# Load scribe results
with open(
    f"{OUTPUT_DIR}/scribe_zinbvcp_mix_results_"
    f"{n_cells}cells_"
    f"{n_genes}genes_"
    f"{n_shared_genes}shared_"
    f"{n_unique_genes}unique_"
...
mean_shared = np.median(shared_genes, axis=0)
mean_unique = np.median(unique_genes, axis=0)

# Get indices where mean counts > 0
nonzero_idx_shared = np.where(mean_shared > 0)[0]
nonzero_idx_unique = np.where(mean_unique > 0)[0]

# Sort the nonzero means and get sorting indices
sorted_idx_shared = nonzero_idx_shared[np.argsort(mean_shared[nonzero_idx_shared])]
sorted_idx_unique = nonzero_idx_unique[np.argsort(mean_unique[nonzero_idx_unique])]

# Generate evenly spaced indices across the sorted nonzero genes
spaced_indices_shared = np.linspace(0, len(sorted_idx_shared)-1, num=n_genes, dtype=int)
spaced_indices_unique = np.linspace(0, len(sorted_idx_unique)-1, num=n_genes, dtype=int)

# Get the actual gene indices after sorting
selected_idx_shared = sorted_idx_shared[spaced_indices_shared]
selected_idx_unique = sorted_idx_unique[spaced_indices_unique]
