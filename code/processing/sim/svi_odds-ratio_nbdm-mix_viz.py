...
# Define prefix
prefix = f"{n_components}components_{n_genes}genes_{n_cells}cells_{n_steps}steps"
...
# Save figure
fig.savefig(
    f"{FIG_DIR}/{prefix}_ppc_shared_{parameterization}.png", 
    bbox_inches="tight"
)
...
# Save figure
fig.savefig(
    f"{FIG_DIR}/{prefix}_ppc_unique_{parameterization}.png", 
    bbox_inches="tight"
)
...
# Compute the KL divergence between the posterior lognormal distributions for
# the mu parameters

mu_mu_1 = results.params["mu_loc"][0, :]
sigma_mu_1 = results.params["mu_scale"][0, :]

mu_mu_2 = results.params["mu_loc"][1, :]
sigma_mu_2 = results.params["mu_scale"][1, :]

kl_divergence = scribe.stats.kl_lognormal(
    mu_mu_1, sigma_mu_1, mu_mu_2, sigma_mu_2
)
...
ax.set_ylabel("ECDF")
ax.set_title(r"KL divergence for $\mu$ parameters")
...
# Save figure
fig.savefig(
    f"{FIG_DIR}/{prefix}_kl_divergence_mu_{parameterization}.png", 
    bbox_inches="tight"
)
