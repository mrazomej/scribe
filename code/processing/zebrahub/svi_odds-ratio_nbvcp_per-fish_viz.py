...
model_files = sorted(glob.glob(f"{OUTPUT_DIR}/svi_*.pkl"))
...
for i, model_file in enumerate(model_files):
    ...
    print(
        f"Plotting results for dataset {dataset_name} ({i+1} of {len(model_files)})..."
    )
    ...
    # Save figure
    fig.savefig(
        f"{FIG_DIR}/{dataset_name}_{parameterization.replace('_', '-')}_loss_history.png",
        bbox_inches="tight",
    )
    ...
    fig.savefig(
        f"{FIG_DIR}/{dataset_name}_{parameterization.replace('_', '-')}_example_ECDF.png",
        bbox_inches="tight",
    )
    ...
    for j, ax in enumerate(axes):
        ...
        credible_regions = scribe.stats.compute_histogram_credible_regions(
            results_subset.predictive_samples[:, :, j],
    ...
    # Save figure
    fig.savefig(
        f"{FIG_DIR}/{dataset_name}_{parameterization.replace('_', '-')}_example_ppc.png",
        bbox_inches="tight",
    )
