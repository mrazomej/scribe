...
model combines aspects of both the [ZINB](zinb.md) and [NBVCP](nbvcp.md)
models to handle both technical dropouts and variable capture efficiencies in
single-cell RNA sequencing data. This model is particularly useful when the
data exhibits both excess zeros and significant variation in total UMI counts
across cells.

The ZINBVCP model incorporates two key features:

1. Zero-inflation to model technical dropouts (from [ZINB](zinb.md))
2. Cell-specific capture probabilities (from [NBVCP](nbvcp.md))

## Model Comparison with NBVCP and ZINB

The ZINBVCP model extends both the [NBVCP](nbvcp.md) and [ZINB](zinb.md)
models by combining their key features. From the [NBVCP](nbvcp.md) model, it
inherits the cell-specific capture probabilities \(\nu^{(c)}\) that modify the
base success probability \(p\). From the [ZINB](zinb.md) model, it inherits
the gene-specific dropout probabilities \(\pi_g\) that model technical zeros.
...
1. Each gene has an associated dropout probability \(\pi_g\)
2. Each cell has an associated capture probability \(\nu^{(c)}\)
3. The base success probability \(p\) is modified by each cell's capture
   probability
...
The ZINBVCP model combines the derivations of the [NBVCP](nbvcp.md) and
[ZINB](zinb.md) models. Starting with the standard negative binomial model for
mRNA counts:
...
where \(z_g^{(c)} \sim \text{Bernoulli}(\pi_g)\). Marginalizing over the
unobserved mRNA counts \(m_g^{(c)}\) and dropout indicators \(z_g^{(c)}\), we
get:
...
where \(\hat{p}^{(c)}\) is the effective probability defined in Eq. (1) and
\(\delta_0\) is the Dirac delta function at zero.
...
The training process follows similar steps to the [NBVCP](nbvcp.md) and
[ZINB](zinb.md) models:
...
1. Initialize variational parameters:
    ...
    - \(\hat{\alpha}_{\pi,g} = \alpha_{\pi}\),
      \(\hat{\beta}_{\pi,g} = \beta_{\pi}\) for all genes \(g\)
    - \(\hat{\alpha}_{\nu}^{(c)} = \alpha_{\nu}\),
      \(\hat{\beta}_{\nu}^{(c)} = \beta_{\nu}\) for all cells \(c\)
...
[NumPyro](https://num.pyro.ai/en/stable/index.html) with key features
including:
...
- Variation in total UMI counts partially reflects technical capture
  differences
...
The model provides the most comprehensive treatment of technical artifacts
among the non-mixture models in SCRIBE, accounting for both dropouts and
capture efficiency variation. However, this flexibility comes at the cost of
increased model complexity and computational demands.
