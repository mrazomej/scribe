...
def test_pln_vae_get_compositional_samples_and_empirical_de():
    """End-to-end: PLN VAE fit → compositional sampling → empirical DE.

    Confirms the API parity added in the merge-readiness pass:
    PLN VAE results now expose ``get_pln_compositional_samples``
    and the unprefixed ``get_compositional_samples`` on
    ScribeVAEResults so ``compare(method='empirical')`` works
    on VAE-fit LNM/PLN results, mirroring the Laplace path.
    """
    import anndata as ad
    import numpy as np
    import scribe
    ...
    n_cells, n_genes, k = 32, 12, 3
    counts = np.asarray(
        random.poisson(random.PRNGKey(30), 5.0, shape=(n_cells, n_genes))
    ).astype(np.float32)
    adata = ad.AnnData(counts)
    ...
    # Compositional samples: simplex valid, shape correct.
    ...
    # Model-prefixed accessor: same result as unprefixed.
    cs_pref = res_A.get_pln_compositional_samples(
        n_samples=64, rng_key=random.PRNGKey(0), store_samples=False
    )
    assert cs_pref.shape == (64, n_genes)

    # Empirical DE through compare()
