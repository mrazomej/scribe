...
def test_log_likelihood(example_zinbvcp_results, small_dataset, rng_key):
    ...
    cell_log_liks = example_zinbvcp_results.compute_log_likelihood(
        counts=counts, 
        cells_axis=0,
        return_by='cell'
    )
    ...
    assert cell_log_liks.shape[1] == example_zinbvcp_results.n_cells  # n_cells
    
    # Test log likelihood evaluation - with gene axis
    gene_log_liks = example_zinbvcp_results.compute_log_likelihood(
        counts=counts, 
        cells_axis=0,
        return_by='gene'
    )
    ...
    assert gene_log_liks.shape[1] == example_zinbvcp_results.n_genes  # n_genes
...
def test_log_likelihood_batching(example_zinbvcp_results, small_dataset, rng_key):
    ...
    full_log_liks = example_zinbvcp_results.compute_log_likelihood(
        counts=counts, 
        cells_axis=0,
        return_by='cell'
    )
    ...
    batched_log_liks = example_zinbvcp_results.compute_log_likelihood(
        counts=counts, 
