...
import numpyro
from scribe.models_mix import nbdm_mixture_model, nbdm_mixture_guide
...
from scribe.results import NBDMMixtureResults
...
def test_inference_run(small_dataset, rng_key):
    ...
    assert isinstance(results, NBDMMixtureResults)
...
def test_parameter_ranges(example_nbdm_mix_results):
    ...
    assert jnp.all(example_nbdm_mix_results.params['alpha_p'] > 0)
    assert jnp.all(example_nbdm_mix_results.params['beta_p'] > 0)
    assert jnp.all(example_nbdm_mix_results.params['alpha_r'] > 0)
    assert jnp.all(example_nbdm_mix_results.params['beta_r'] > 0)
    assert jnp.all(example_nbdm_mix_results.params['alpha_mixing'] > 0)

    # Check shapes of component-specific parameters
    assert example_nbdm_mix_results.params['alpha_mixing'].shape == (N_COMPONENTS,)
    assert example_nbdm_mix_results.params['alpha_r'].shape == (N_COMPONENTS, example_nbdm_mix_results.n_genes)
    assert example_nbdm_mix_results.params['beta_r'].shape == (N_COMPONENTS, example_nbdm_mix_results.n_genes)

def test_continue_training(example_nbdm_mix_results, small_dataset, rng_key):
    ...
    results = rerun_scribe(
        results=example_nbdm_mix_results,
        counts=counts,
        rng_key=rng_key,
        n_steps=N_STEPS,
        batch_size=5,
...
def test_posterior_sampling(example_nbdm_mix_results, rng_key):
    ...
    n_samples = 100
    samples = example_nbdm_mix_results.get_posterior_samples(
        rng_key=rng_key,
        n_samples=n_samples
    )
    ...
    assert samples['r'].shape == (n_samples, N_COMPONENTS, example_nbdm_mix_results.n_genes)
...
def test_predictive_sampling(example_nbdm_mix_results, rng_key):
    ...
    predictive_samples = example_nbdm_mix_results.get_predictive_samples(
        rng_key=random.split(rng_key)[1]
    )
    
    expected_shape = (
        n_samples,
        example_nbdm_mix_results.n_cells,
        example_nbdm_mix_results.n_genes
    )
...
def test_ppc_sampling(example_nbdm_mix_results, rng_key):
    ...
    ppc_samples = example_nbdm_mix_results.get_ppc_samples(
        rng_key=rng_key,
        n_samples=n_samples
    )
...
def test_get_distributions(example_nbdm_mix_results):
    ...
    distributions = example_nbdm_mix_results.get_distributions()
    
    assert 'mixing_weights' in distributions
    assert 'p' in distributions
    assert 'r' in distributions

def test_indexing(example_nbdm_mix_results):
    """Test indexing functionality."""
    # Single gene
    subset = example_nbdm_mix_results[0]
    assert isinstance(subset, NBDMMixtureResults)
...
def test_parameter_subsetting(example_nbdm_mix_results):
...
def test_log_likelihood(example_nbdm_mix_results, small_dataset, rng_key):
    ...
    cell_log_liks = example_nbdm_mix_results.compute_log_likelihood(
        counts=counts, 
        cells_axis=0,
        return_by='cell',
    ...
    assert cell_log_liks.shape[1] == example_nbdm_mix_results.n_cells  # n_cells
    
    # Test with split_components=True
    cell_log_liks_split = example_nbdm_mix_results.compute_log_likelihood(
        counts=counts, 
        cells_axis=0,
        return_by='cell',
    ...
    assert cell_log_liks_split.shape[1] == example_nbdm_mix_results.n_cells  # n_cells
    assert cell_log_liks_split.shape[2] == N_COMPONENTS  # n_components
    
    # Test gene-wise likelihood without split_components
    gene_log_liks = example_nbdm_mix_results.compute_log_likelihood(
        counts=counts, 
        cells_axis=0,
        return_by='gene',
        split_components=False
    ...
    assert gene_log_liks.shape[1] == example_nbdm_mix_results.n_genes  # n_genes
    
    # Test gene-wise likelihood with split_components
    gene_log_liks_split = example_nbdm_mix_results.compute_log_likelihood(
        counts=counts, 
        cells_axis=0,
        return_by='gene',
        split_components=True
    ...
    assert gene_log_liks_split.shape[1] == example_nbdm_mix_results.n_genes  # n_genes
...
def test_log_likelihood_batching(example_nbdm_mix_results, small_dataset, rng_key):
    ...
    full_log_liks = example_nbdm_mix_results.compute_log_likelihood(
        counts=counts, 
        cells_axis=0,
        return_by='cell',
        split_components=False
    ...
    batched_log_liks = example_nbdm_mix_results.compute_log_likelihood(
        counts=counts, 
    ...
    # Compute log likelihood without batching
    full_log_liks_split = example_nbdm_mix_results.compute_log_likelihood(
        counts=counts, 
        cells_axis=0,
        return_by='cell',
        split_components=True
    ...
    # Compute log likelihood with batching
    batched_log_liks_split = example_nbdm_mix_results.compute_log_likelihood(
        counts=counts, 
        batch_size=5,  # Small batch size for testing
        cells_axis=0,
        return_by='cell',
