...
def nbvcp_mix_results(
    ...
    if parameterization == "standard":
        ...
        priors = {"p": (1, 1), "mu": (1, 1), "p_capture": (1, 1)}
    elif parameterization == "odds_ratio":
        priors = {"phi": (3, 2), "mu": (1, 1), "phi_capture": (3, 2)}
    else:
...
def test_parameterization_config(
    ...
    if guide_rank is not None:
        ...
        assert nbvcp_mix_results.model_config.guide_families is not None
        guide_family = nbvcp_mix_results.model_config.guide_families.get(
            gene_param
        )
...
def test_posterior_sampling(
    ...
    if parameterization == "standard":
        if unconstrained:
            ...
            if "r" in samples:
                assert samples["r"].shape[-2] == nbvcp_mix_results.n_components
            ...
            if "p_capture" in samples:
                assert (
                    samples["p_capture"].shape[-1] == nbvcp_mix_results.n_cells
                )
        ...
        if unconstrained:
            ...
            if len(map_est) == 0:
                ...
                if hasattr(nbvcp_mix_results, "params"):
                    print(
                        f"DEBUG: Model params keys: {list(nbvcp_mix_results.params.keys())}"
                    )
...
def test_cell_specific_parameters(
    ...
    if parameterization == "odds_ratio":
        if unconstrained:
            # For unconstrained models, check if we have either constrained or unconstrained phi_capture parameters
            has_phi_capture = (
                "phi_capture" in params or "phi_capture_unconstrained" in params
            )
            if has_phi_capture:
                if "phi_capture" in params:
                    ...
                    # In unconstrained models, phi_capture is transformed from unconstrained space
                    # but should still be finite and cell-specific
                    assert (
                        phi_capture.shape[-1] == nbvcp_mix_results.n_cells
                    ), "phi_capture should be cell-specific"
                    assert jnp.all(jnp.isfinite(phi_capture))
                # Note: phi_capture_unconstrained parameters don't have range constraints
        else:
            # For constrained models, phi_capture parameters must be positive
            if "phi_capture" in params:
                phi_capture = params["phi_capture"]
                assert (
                    phi_capture.shape[-1] == nbvcp_mix_results.n_cells
                ...
                assert jnp.all(
                    phi_capture > 0
                ), "phi_capture should be positive"
    else:
        if unconstrained:
            # For unconstrained models, check if we have either constrained or unconstrained p_capture parameters
            has_p_capture = (
                "p_capture" in params or "p_capture_unconstrained" in params
            )
            if has_p_capture:
                if "p_capture" in params:
                    p_capture = params["p_capture"]
                    # In unconstrained models, p_capture is transformed from unconstrained space
                    # but should still be finite and cell-specific
                    assert (
                        p_capture.shape[-1] == nbvcp_mix_results.n_cells
                    ), "p_capture should be cell-specific"
                    assert jnp.all(jnp.isfinite(p_capture))
                # Note: p_capture_unconstrained parameters don't have range constraints
        else:
            # For constrained models, p_capture parameters must be in [0, 1]
            if "p_capture" in params:
                p_capture = params["p_capture"]
                assert (
                    p_capture.shape[-1] == nbvcp_mix_results.n_cells
...
def test_component_selection_with_posterior_samples(
    ...
    if parameterization == "standard":
        if unconstrained:
            ...
            assert (
                has_p and has_r and has_p_capture
            ), f"Expected p/p_unconstrained, r/r_unconstrained, and p_capture/p_capture_unconstrained in component samples, got {list(component.posterior_samples.keys())}"
        ...
        if unconstrained:
            ...
            assert (
                has_p and has_mu and has_p_capture
            ), f"Expected p/p_unconstrained, mu/mu_unconstrained, and p_capture/p_capture_unconstrained in component samples, got {list(component.posterior_samples.keys())}"
            # Check shapes - component dimension should be removed
            if "p" in component.posterior_samples:
                assert component.posterior_samples["p"].shape == (
                    3,
        ...
        if unconstrained:
            ...
            assert (
                has_phi and has_mu and has_phi_capture
            ), f"Expected phi/phi_unconstrained, mu/mu_unconstrained, and phi_capture/phi_capture_unconstrained in component samples, got {list(component.posterior_samples.keys())}"
            # Check shapes - component dimension should be removed
...
def test_subset_with_posterior_samples(
    ...
    if parameterization == "standard":
        if unconstrained:
            ...
            assert (
                has_p and has_r and has_p_capture and has_mixing
            ), f"Expected p/p_unconstrained, r/r_unconstrained, p_capture/p_capture_unconstrained, and mixing_weights/mixing_logits_unconstrained in subset samples, got {list(subset.posterior_samples.keys())}"
        ...
        if unconstrained:
            ...
            assert (
                has_p and has_mu and has_p_capture and has_mixing
            ), f"Expected p/p_unconstrained, mu/mu_unconstrained, p_capture/p_capture_unconstrained, and mixing_weights/mixing_logits_unconstrained in subset samples, got {list(subset.posterior_samples.keys())}"
        ...
        if unconstrained:
            ...
            assert (
                has_phi and has_mu and has_phi_capture and has_mixing
            ), f"Expected phi/phi_unconstrained, mu/mu_unconstrained, phi_capture/phi_capture_unconstrained, and mixing_weights/mixing_logits_unconstrained in subset samples, got {list(subset.posterior_samples.keys())}"
