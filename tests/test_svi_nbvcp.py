...
def test_model_and_guide_retrieval(example_nbvcp_results):
    ...
    model, guide = example_nbvcp_results._model_and_guide()
...
def test_log_likelihood_fn_retrieval(example_nbvcp_results):
    ...
    log_lik_fn = example_nbvcp_results._log_likelihood_fn()
...
def test_compute_log_likelihood(example_nbvcp_results, small_dataset, rng_key):
    ...
    cell_ll = example_nbvcp_results.log_likelihood(
        counts,
        return_by='cell'
    )
    ...
    gene_ll = example_nbvcp_results.log_likelihood(
        counts,
        return_by='gene'
    )
...
def test_compute_log_likelihood_batched(example_nbvcp_results, small_dataset, rng_key):
    ...
    full_ll = example_nbvcp_results.log_likelihood(
        counts,
        return_by='cell'
    )
    
    # Compute with batching
    batched_ll = example_nbvcp_results.log_likelihood(
        counts,
        batch_size=3,
        return_by='cell'
    )
