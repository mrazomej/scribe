...
def test_parameterization_config(
    nbvcp_results, parameterization, unconstrained, guide_rank
):
    ...
    if guide_rank is not None:
        ...
        if parameterization == "standard":
            ...
            gene_param = "mu"
        assert nbvcp_results.model_config.guide_families is not None
        guide_family = nbvcp_results.model_config.guide_families.get(gene_param)
    ...
    if parameterization == "standard":
        ...
        if unconstrained:
            ...
            if "p" in params and "r" in params and "p_capture" in params:
                ...
                # In unconstrained models, p, r, and p_capture are transformed
        ...
        if unconstrained:
            ...
            if "p" in params and "mu" in params and "p_capture" in params:
                ...
                # In unconstrained models, p, mu, and p_capture are transformed
                # from unconstrained space but should still be finite
                assert jnp.all(jnp.isfinite(p))
        ...
        if unconstrained:
            ...
            if "phi" in params and "mu" in params and "phi_capture" in params:
                ...
                # In unconstrained models, phi, mu, and phi_capture are
                # transformed from unconstrained space but should still be
                # finite
...
def test_posterior_sampling(
    nbvcp_results, rng_key, parameterization, unconstrained, guide_rank
):
...
def test_get_map(nbvcp_results, parameterization, unconstrained, guide_rank):
    ...
    if parameterization == "standard":
        if unconstrained:
            # For SVI unconstrained models, they might return empty dicts
            # initially This appears to be expected behavior for some
            ...
            if len(map_est) == 0:
                ...
                if hasattr(nbvcp_results, "params"):
                    print(
                        ...
                        f"{list(nbvcp_results.params.keys())}"
                    )
...
def test_mcmc_samples_shape(
    nbvcp_results, inference_method, parameterization, unconstrained, guide_rank
):
...
def test_unconstrained_parameter_handling(
    nbvcp_results, unconstrained, guide_rank
):
...
def test_low_rank_guide_params(nbvcp_results, inference_method, guide_rank):
    ...
    if inference_method == "svi" and guide_rank is not None:
        ...
        assert hasattr(nbvcp_results, "params")
        params = nbvcp_results.params
...
def test_low_rank_covariance_structure(
    nbvcp_results, inference_method, guide_rank, parameterization, rng_key
):
    """
    Test that low-rank guides produce samples with appropriate covariance
    structure.
    """
    if inference_method == "svi" and guide_rank is not None:
