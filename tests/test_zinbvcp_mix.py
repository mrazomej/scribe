# tests/test_zinbvcp_mix.py
"""
Tests for the Zero-Inflated Negative Binomial Mixture Model with Variable Capture
Probability.
"""
import pytest
...
import numpyro
from scribe.models_mix import zinbvcp_mixture_model, zinbvcp_mixture_guide, zinbvcp_mixture_log_likelihood
...
def example_zinbvcp_mix_results(small_dataset, rng_key):
    """Generate example ZINBVCP mixture model results for testing."""
...
def test_inference_run(small_dataset, rng_key):
    ...
    assert results.model_type == "zinbvcp_mix"
    assert isinstance(results.model_config, ModelConfig)

def test_parameter_ranges(example_zinbvcp_mix_results):
    """Test that inferred parameters are in valid ranges."""
    # All parameters should be positive - look for distribution parameters
    r_params = [
        param for param in example_zinbvcp_mix_results.params 
        if param.startswith('r_')
    ]
    for param in r_params:
        assert jnp.all(example_zinbvcp_mix_results.params[param] > 0)
        assert example_zinbvcp_mix_results.params[param].shape == (N_COMPONENTS, example_zinbvcp_mix_results.n_genes)
        
    # Check p parameters - should be positive
    p_params = [param for param in example_zinbvcp_mix_results.params if param.startswith('p_')
                and not param.startswith('p_capture_')]
    for param in p_params:
        assert jnp.all(example_zinbvcp_mix_results.params[param] > 0)
        
    # Check gate parameters - should be positive
    gate_params = [param for param in example_zinbvcp_mix_results.params if param.startswith('gate_')]
    for param in gate_params:
        assert jnp.all(example_zinbvcp_mix_results.params[param] > 0)
        assert example_zinbvcp_mix_results.params[param].shape == (N_COMPONENTS, example_zinbvcp_mix_results.n_genes)
        
    # Check p_capture parameters - should be positive and cell-specific
    p_capture_params = [param for param in example_zinbvcp_mix_results.params if param.startswith('p_capture_')]
    for param in p_capture_params:
        assert jnp.all(example_zinbvcp_mix_results.params[param] > 0)
        assert example_zinbvcp_mix_results.params[param].shape == (example_zinbvcp_mix_results.n_cells,)
        
    # Check mixing parameters - should be positive
    mixing_params = [param for param in example_zinbvcp_mix_results.params if param.startswith('mixing_')]
    for param in mixing_params:
        assert jnp.all(example_zinbvcp_mix_results.params[param] > 0)
        assert example_zinbvcp_mix_results.params[param].shape == (N_COMPONENTS,)
...
def test_get_model_and_guide():
    ...
    model, guide = get_model_and_guide("zinbvcp_mix")
    
    assert model == zinbvcp_mixture_model
    assert guide == zinbvcp_mixture_guide

def test_get_log_likelihood_fn():
    """Test that get_log_likelihood_fn returns the correct function."""
    log_lik_fn = get_log_likelihood_fn("zinbvcp_mix")
    
    assert log_lik_fn == zinbvcp_mixture_log_likelihood

# ------------------------------------------------------------------------------
# Test sampling
# ------------------------------------------------------------------------------

def test_posterior_sampling(example_zinbvcp_mix_results, rng_key):
    """Test sampling from the variational posterior."""
    n_samples = 10
    posterior_samples = example_zinbvcp_mix_results.get_posterior_samples(
    ...
    assert posterior_samples['r'].shape == (n_samples, N_COMPONENTS, example_zinbvcp_mix_results.n_genes)
    assert posterior_samples['gate'].shape == (n_samples, N_COMPONENTS, example_zinbvcp_mix_results.n_genes)
    assert posterior_samples['p_capture'].shape == (n_samples, example_zinbvcp_mix_results.n_cells)
...
def test_predictive_sampling(example_zinbvcp_mix_results, rng_key):
    ...
    pred_samples = example_zinbvcp_mix_results.get_predictive_samples(
        rng_key=random.PRNGKey(43),
        store_samples=True
    )
    
    expected_shape = (n_samples, example_zinbvcp_mix_results.n_cells, example_zinbvcp_mix_results.n_genes)
    assert pred_samples.shape == expected_shape
    assert jnp.all(pred_samples >= 0)  # Counts should be non-negative

def test_ppc_sampling(example_zinbvcp_mix_results, rng_key):
    """Test posterior predictive check sampling."""
    n_samples = 10
    
    ppc_results = example_zinbvcp_mix_results.get_ppc_samples(
...
def test_get_distributions(example_zinbvcp_mix_results):
    """Test getting distribution objects from results."""
    # Get scipy distributions
    scipy_dists = example_zinbvcp_mix_results.get_distributions(backend="scipy")
    ...
    numpyro_dists = example_zinbvcp_mix_results.get_distributions(backend="numpyro")
...
def test_get_map(example_zinbvcp_mix_results):
    """Test getting MAP estimates."""
    map_estimates = example_zinbvcp_mix_results.get_map()
    ...
    assert map_estimates['r'].shape == (N_COMPONENTS, example_zinbvcp_mix_results.n_genes)
    assert map_estimates['gate'].shape == (N_COMPONENTS, example_zinbvcp_mix_results.n_genes)
    assert map_estimates['p_capture'].shape == (example_zinbvcp_mix_results.n_cells,)
    assert map_estimates['mixing_weights'].shape == (N_COMPONENTS,)

def test_model_and_guide_retrieval(example_zinbvcp_mix_results):
    """Test that model and guide functions can be retrieved from results."""
    model, guide = example_zinbvcp_mix_results.get_model_and_guide()
    
    assert callable(model)
    assert callable(guide)

def test_log_likelihood_fn_retrieval(example_zinbvcp_mix_results):
    """Test that log likelihood function can be retrieved from results."""
    log_lik_fn = example_zinbvcp_mix_results.get_log_likelihood_fn()
...
def test_getitem_integer(example_zinbvcp_mix_results):
    """Test indexing with an integer."""
    subset = example_zinbvcp_mix_results[0]
    
    assert subset.n_genes == 1
    assert subset.n_cells == example_zinbvcp_mix_results.n_cells
    assert subset.model_type == example_zinbvcp_mix_results.model_type
    assert subset.n_components == N_COMPONENTS
    
    # Check component and gene-specific parameters are correctly subset
    for param in example_zinbvcp_mix_results.params:
    ...
    for param in example_zinbvcp_mix_results.params:
        if param.startswith('p_capture_'):
            ...
            assert subset.params[param].shape == (example_zinbvcp_mix_results.n_cells,)

def test_getitem_slice(example_zinbvcp_mix_results):
    """Test indexing with a slice."""
    subset = example_zinbvcp_mix_results[0:2]
...
def test_getitem_boolean(example_zinbvcp_mix_results):
    """Test indexing with a boolean array."""
    mask = jnp.array([True, False, True] + [False] * (example_zinbvcp_mix_results.n_genes - 3))
    subset = example_zinbvcp_mix_results[mask]
...
def test_subset_with_posterior_samples(example_zinbvcp_mix_results, rng_key):
    ...
    assert subset.posterior_samples['p_capture'].shape == (N_SAMPLES, example_zinbvcp_mix_results.n_cells)
...
def test_get_component(example_zinbvcp_mix_results):
    """Test selecting a specific component from a mixture model."""
    # Select the first component
    component = example_zinbvcp_mix_results.get_component(0)
    
    # Check that it's no longer a mixture model
    assert component.n_components is None
    assert component.model_type == "zinbvcp"  # Model type should lose _mix suffix
    
    # Check that gene and cell counts are preserved
    assert component.n_genes == example_zinbvcp_mix_results.n_genes
    assert component.n_cells == example_zinbvcp_mix_results.n_cells
    ...
    for param in example_zinbvcp_mix_results.params:
        if param.startswith('r_') or param.startswith('gate_'):
            ...
            orig_shape = example_zinbvcp_mix_results.params[param].shape
            if len(orig_shape) > 1 and orig_shape[0] == N_COMPONENTS:
                assert component.params[param].shape == (example_zinbvcp_mix_results.n_genes,)
                # Check values match
                assert jnp.allclose(
                    component.params[param],
                    example_zinbvcp_mix_results.params[param][0]
                )
    ...
    for param in example_zinbvcp_mix_results.params:
        if param.startswith('p_capture_'):
            ...
            assert component.params[param].shape == (example_zinbvcp_mix_results.n_cells,)
            assert jnp.allclose(
                component.params[param],
                example_zinbvcp_mix_results.params[param]
            )

def test_get_component_with_posterior_samples(example_zinbvcp_mix_results, rng_key):
    ...
    # Select component
    component = example_zinbvcp_mix_results.get_component(1)
    ...
    assert component.posterior_samples['r'].shape == (N_SAMPLES, example_zinbvcp_mix_results.n_genes)
    assert component.posterior_samples['gate'].shape == (N_SAMPLES, example_zinbvcp_mix_results.n_genes)
    assert component.posterior_samples['p_capture'].shape == (N_SAMPLES, example_zinbvcp_mix_results.n_cells)
    
    # Check values match the original component values
    assert jnp.allclose(
        component.posterior_samples['r'],
        example_zinbvcp_mix_results.posterior_samples['r'][:, 1, :]
    )
    assert jnp.allclose(
        component.posterior_samples['gate'],
        example_zinbvcp_mix_results.posterior_samples['gate'][:, 1, :]
    )
    # Cell-specific parameters should be unmodified
    assert jnp.allclose(
        component.posterior_samples['p_capture'],
        example_zinbvcp_mix_results.posterior_samples['p_capture']
    )

def test_component_then_gene_indexing(example_zinbvcp_mix_results):
    ...
    assert gene_subset.n_cells == example_zinbvcp_mix_results.n_cells
    ...
    for param in component.params:
        if param.startswith('r_') or param.startswith('gate_'):
            ...
            if len(orig_shape) > 0 and orig_shape[0] == example_zinbvcp_mix_results.n_genes:
                assert gene_subset.params[param].shape == (2,)
    
    # Check cell parameters are unchanged
    for param in component.params:
        if param.startswith('p_capture_'):
            assert gene_subset.params[param].shape == (example_zinbvcp_mix_results.n_cells,)

# ------------------------------------------------------------------------------
# Test log likelihood
# ------------------------------------------------------------------------------

def test_compute_log_likelihood(example_zinbvcp_mix_results, small_dataset, rng_key):
    ...
    cell_ll = example_zinbvcp_mix_results.compute_log_likelihood(
        counts,
        return_by='cell',
    ...
    assert cell_ll.shape == (N_SAMPLES, example_zinbvcp_mix_results.n_cells)
    ...
    gene_ll = example_zinbvcp_mix_results.compute_log_likelihood(
        counts,
        return_by='gene',
        split_components=False
    )
    ...
    assert gene_ll.shape == (N_SAMPLES, example_zinbvcp_mix_results.n_genes)
    assert jnp.all(jnp.isfinite(gene_ll))
    
    # Now test with split_components=True
    # Compute per-cell log likelihood by component
    cell_ll_by_comp = example_zinbvcp_mix_results.compute_log_likelihood(
        counts,
        return_by='cell',
    ...
    assert cell_ll_by_comp.shape == (N_SAMPLES, example_zinbvcp_mix_results.n_cells, N_COMPONENTS)
    assert jnp.all(jnp.isfinite(cell_ll_by_comp))
    
    # Compute per-gene log likelihood by component
    gene_ll_by_comp = example_zinbvcp_mix_results.compute_log_likelihood(
        counts,
        return_by='gene',
        split_components=True
    )
    ...
    assert gene_ll_by_comp.shape == (N_SAMPLES, example_zinbvcp_mix_results.n_genes, N_COMPONENTS)
    assert jnp.all(jnp.isfinite(gene_ll_by_comp))

def test_compute_log_likelihood_batched(example_zinbvcp_mix_results, small_dataset, rng_key):
    ...
    full_ll = example_zinbvcp_mix_results.compute_log_likelihood(
        counts,
        return_by='cell',
        split_components=False
    )
    ...
    batched_ll = example_zinbvcp_mix_results.compute_log_likelihood(
        counts,
        batch_size=3,
        return_by='cell',
        split_components=False
    ...
    full_ll_split = example_zinbvcp_mix_results.compute_log_likelihood(
        counts,
        return_by='cell',
        split_components=True
    )
    ...
    # Compute with batching
    batched_ll_split = example_zinbvcp_mix_results.compute_log_likelihood(
        counts,
        batch_size=3,
        return_by='cell',
        split_components=True
...
def test_compute_cell_type_assignments(example_zinbvcp_mix_results, small_dataset, rng_key):
    ...
    # Compute assignments
    assignments = example_zinbvcp_mix_results.compute_cell_type_assignments(
        counts,
    ...
    # Check shapes
    assert assignments['concentration'].shape == (example_zinbvcp_mix_results.n_cells, N_COMPONENTS)
    assert assignments['mean_probabilities'].shape == (example_zinbvcp_mix_results.n_cells, N_COMPONENTS)
    assert assignments['sample_probabilities'].shape == (N_SAMPLES, example_zinbvcp_mix_results.n_cells, N_COMPONENTS)
    ...
    assignments_no_fit = example_zinbvcp_mix_results.compute_cell_type_assignments(
        counts,
    ...
    assert assignments_no_fit['sample_probabilities'].shape == (N_SAMPLES, example_zinbvcp_mix_results.n_cells, N_COMPONENTS)

def test_compute_cell_type_assignments_map(example_zinbvcp_mix_results, small_dataset):
    """Test computing cell type assignments using MAP estimates."""
    counts, _ = small_dataset
    
    # Compute assignments using MAP
    assignments = example_zinbvcp_mix_results.compute_cell_type_assignments_map(
        counts,
        verbose=False
    )
    ...
    # Check shapes
    assert assignments['probabilities'].shape == (example_zinbvcp_mix_results.n_cells, N_COMPONENTS)