...
def zinbvcp_mix_results(
    ...
    _zinbvcp_mix_results_cache[key] = result
    return result
...
def test_inference_run(zinbvcp_mix_results, guide_rank):
    ...
    assert zinbvcp_mix_results.model_type == "zinbvcp_mix"
...
def test_parameterization_config(
    ...
    if True:  # Always check unconstrained by type
        assert (
            isinstance(
                zinbvcp_mix_results.model_config, UnconstrainedModelConfig
            )
            == unconstrained
        )
...
def test_parameter_ranges(
    ...
    if hasattr(zinbvcp_mix_results, "params") and hasattr(
...
def test_predictive_sampling(zinbvcp_mix_results, rng_key, guide_rank):
    ...
    if hasattr(zinbvcp_mix_results, "get_posterior_samples"):
        ...
        if hasattr(zinbvcp_mix_results, "get_samples"):  # MCMC case
            ...
            pred = zinbvcp_mix_results.get_ppc_samples(
            ...
            pred = zinbvcp_mix_results.get_predictive_samples(
                rng_key=rng_key, store_samples=True
            )
    else:
        pred = zinbvcp_mix_results.get_ppc_samples(
    ...
    assert pred.shape[-1] == zinbvcp_mix_results.n_genes
...
def test_get_map(
    ...
    map_est = (
        ...
        if hasattr(zinbvcp_mix_results, "get_map")
        else None
    )
...
def test_indexing_integer(zinbvcp_mix_results, guide_rank):
    ...
    assert subset.n_cells == zinbvcp_mix_results.n_cells
...
def test_indexing_boolean(zinbvcp_mix_results, guide_rank):
    ...
    subset = zinbvcp_mix_results[mask]
    assert subset.n_genes == 1
    assert subset.n_cells == zinbvcp_mix_results.n_cells
    assert subset.n_components == zinbvcp_mix_results.n_components
...
def test_log_likelihood(
    ...
    ll = zinbvcp_mix_results.log_likelihood(counts, return_by="cell")
    assert ll.shape[0] > 0
    assert jnp.all(jnp.isfinite(ll))

    # Test component-specific log likelihood
    ll_components = zinbvcp_mix_results.log_likelihood(
        counts, return_by="cell", split_components=True
    )
    assert ll_components.shape[-1] == zinbvcp_mix_results.n_components
...
def test_gene_specific_parameters(
    ...
    if "gate" in params:
        gate = params["gate"]
        assert (
            gate.shape[-1] == zinbvcp_mix_results.n_genes
...
def test_cell_specific_parameters(
    ...
    if parameterization == "odds_ratio":
        if "phi_capture" in params:
            ...
            assert (
                phi_capture.shape[-1] == zinbvcp_mix_results.n_cells
        ...
        if "p_capture" in params:
            ...
            assert (
                p_capture.shape[-1] == zinbvcp_mix_results.n_cells
...
def test_zero_inflation_behavior(zinbvcp_mix_results, rng_key, guide_rank):
    ...
    assert jnp.any(
        pred > 0
    ), "ZINBVCP mixture model should also produce non-zero counts"
...
def test_component_selection(zinbvcp_mix_results, guide_rank):
    ...
    assert component.n_genes == zinbvcp_mix_results.n_genes
...
def test_component_selection_with_posterior_samples(
    ...
    # Select component
    component = zinbvcp_mix_results.get_component(1)
...
def test_cell_type_assignments(
    ...
    # Compute assignments
    assignments = zinbvcp_mix_results.cell_type_probabilities(
...
def test_subset_with_posterior_samples(
    ...
    subset = zinbvcp_mix_results[0:2]
