...
import numpyro
from scribe.models_mix import zinbvcp_mixture_model, zinbvcp_mixture_guide
...
from scribe.results import ZINBVCPMixtureResults
...
def test_inference_run(small_dataset, rng_key):
    ...
    assert isinstance(results, ZINBVCPMixtureResults)
...
def test_parameter_ranges(example_zinbvcp_mix_results):
    ...
    assert jnp.all(example_zinbvcp_mix_results.params['alpha_p'] > 0)
    assert jnp.all(example_zinbvcp_mix_results.params['beta_p'] > 0)
    assert jnp.all(example_zinbvcp_mix_results.params['alpha_r'] > 0)
    assert jnp.all(example_zinbvcp_mix_results.params['beta_r'] > 0)
    assert jnp.all(example_zinbvcp_mix_results.params['alpha_gate'] > 0)
    assert jnp.all(example_zinbvcp_mix_results.params['beta_gate'] > 0)
    assert jnp.all(example_zinbvcp_mix_results.params['alpha_p_capture'] > 0)
    assert jnp.all(example_zinbvcp_mix_results.params['beta_p_capture'] > 0)
    assert jnp.all(example_zinbvcp_mix_results.params['alpha_mixing'] > 0)

    # Check shapes of component-specific parameters
    assert example_zinbvcp_mix_results.params['alpha_mixing'].shape == (N_COMPONENTS,)
    assert example_zinbvcp_mix_results.params['alpha_r'].shape == (N_COMPONENTS, example_zinbvcp_mix_results.n_genes)
    assert example_zinbvcp_mix_results.params['beta_r'].shape == (N_COMPONENTS, example_zinbvcp_mix_results.n_genes)
    
    # Check shapes of gate parameters (per component and gene)
    assert example_zinbvcp_mix_results.params['alpha_gate'].shape == (N_COMPONENTS, example_zinbvcp_mix_results.n_genes)
    assert example_zinbvcp_mix_results.params['beta_gate'].shape == (N_COMPONENTS, example_zinbvcp_mix_results.n_genes)
    
    # Check shapes of cell-specific parameters (capture probability parameters are per cell)
    assert example_zinbvcp_mix_results.params['alpha_p_capture'].shape == (example_zinbvcp_mix_results.n_cells,)
    assert example_zinbvcp_mix_results.params['beta_p_capture'].shape == (example_zinbvcp_mix_results.n_cells,)

def test_continue_training(example_zinbvcp_mix_results, small_dataset, rng_key):
    ...
    results = rerun_scribe(
        results=example_zinbvcp_mix_results,
        counts=counts,
        rng_key=rng_key,
        n_steps=N_STEPS,
        batch_size=5,
...
def test_posterior_sampling(example_zinbvcp_mix_results, rng_key):
    ...
    n_samples = 100
    samples = example_zinbvcp_mix_results.get_posterior_samples(
        rng_key=rng_key,
        n_samples=n_samples
    )
    ...
    assert samples['r'].shape == (n_samples, N_COMPONENTS, example_zinbvcp_mix_results.n_genes)
    assert samples['gate'].shape == (n_samples, N_COMPONENTS, example_zinbvcp_mix_results.n_genes)
    assert samples['p_capture'].shape == (n_samples, example_zinbvcp_mix_results.n_cells)
...
def test_predictive_sampling(example_zinbvcp_mix_results, rng_key):
    ...
    predictive_samples = example_zinbvcp_mix_results.get_predictive_samples(
        rng_key=random.split(rng_key)[1]
    )
    
    expected_shape = (
        n_samples,
        example_zinbvcp_mix_results.n_cells,
        example_zinbvcp_mix_results.n_genes
    )
...
def test_ppc_sampling(example_zinbvcp_mix_results, rng_key):
    ...
    ppc_samples = example_zinbvcp_mix_results.get_ppc_samples(
        rng_key=rng_key,
        n_samples=n_samples
    )
...
def test_get_distributions(example_zinbvcp_mix_results):
    ...
    distributions = example_zinbvcp_mix_results.get_distributions()
...
def test_indexing(example_zinbvcp_mix_results):
    """Test indexing functionality."""
    # Single gene
    subset = example_zinbvcp_mix_results[0]
    assert isinstance(subset, ZINBVCPMixtureResults)
...
def test_parameter_subsetting(example_zinbvcp_mix_results):
    ...
    # Gate parameters should maintain component dimension but subset gene dimension
    ...
    assert subset.params['alpha_p_capture'].shape == (example_zinbvcp_mix_results.n_cells,)
    assert subset.params['beta_p_capture'].shape == (example_zinbvcp_mix_results.n_cells,)
