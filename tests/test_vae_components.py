...
class TestSimpleDecoder:
    """Tests for the simple (continuous reconstruction) decoder module."""

    @pytest.fixture
    def decoder(self, input_dim, latent_dim, hidden_dims):
    ...
    @pytest.fixture
    def decoder_params(self, decoder, rng, latent_dim):
        return decoder.init(rng, jnp.zeros(latent_dim))

    def test_output_shape_single(self, decoder, decoder_params, input_dim, latent_dim):
        z = jnp.zeros(latent_dim)
        output = decoder.apply(decoder_params, z)
        assert output.shape == (input_dim,)

    def test_output_shape_batched(
        self, decoder, decoder_params, input_dim, latent_dim
    ):
        z = jnp.zeros((5, latent_dim))
        output = decoder.apply(decoder_params, z)
        assert output.shape == (5, input_dim)

    def test_output_finite(self, decoder, decoder_params, rng, latent_dim):
        z = jax.random.normal(rng, (8, latent_dim))
        output = decoder.apply(decoder_params, z)
        assert jnp.all(jnp.isfinite(output))

    def test_params_structure(self, decoder_params):
        """Decoder should have reversed hidden layers plus output."""
        params = decoder_params["params"]
        # hidden_dims=[64, 32] reversed → [32, 64]
        # So decoder hidden_0 has 32 features, hidden_1 has 64 features
        assert "hidden_0" in params
        assert "hidden_1" in params
    ...
    def test_reversed_architecture(self, rng, input_dim, latent_dim):
        ...
        assert p["output"]["kernel"].shape == (256, input_dim)

    def test_with_destandardization(
        self, rng, input_dim, latent_dim, hidden_dims, standardization_stats
    ):
        """Decoder with destandardization should produce different outputs."""
        mean, std = standardization_stats
        z = jax.random.normal(rng, (3, latent_dim))

        dec_no = SimpleDecoder(
            output_dim=input_dim,
            latent_dim=latent_dim,
            hidden_dims=hidden_dims,
        )
        dec_with = SimpleDecoder(
            output_dim=input_dim,
            latent_dim=latent_dim,
            hidden_dims=hidden_dims,
            standardize_mean=mean,
        ...
        params_no = dec_no.init(rng, jnp.zeros(latent_dim))
        params_with = dec_with.init(rng, jnp.zeros(latent_dim))

        out_no = dec_no.apply(params_no, z)
        out_with = dec_with.apply(params_with, z)

        assert jnp.all(jnp.isfinite(out_with))
        assert not jnp.allclose(out_no, out_with, atol=1e-3)

    def test_deterministic(self, decoder, decoder_params, rng, latent_dim):
        z = jax.random.normal(rng, (4, latent_dim))
        out1 = decoder.apply(decoder_params, z)
        out2 = decoder.apply(decoder_params, z)
...
class TestEncoderDecoderIntegration:
    ...
    def test_round_trip_shapes(
        ...
        assert reconstruction.shape == (8, input_dim)

    def test_round_trip_finite(
        self, encoder_decoder, sample_counts
    ):
    ...
    def test_gradients_flow(self, encoder_decoder, sample_counts):
        ...
        def loss_fn(enc_p, dec_p):
            ...
            return jnp.mean((reconstruction - jnp.log1p(sample_counts)) ** 2)
    ...
    def test_with_standardization_round_trip(
        self, rng, input_dim, latent_dim, hidden_dims, standardization_stats, sample_counts
    ):
        ...
        assert jnp.all(jnp.isfinite(reconstruction))
        assert reconstruction.shape == sample_counts.shape
    ...
    def test_round_trip_with_covariates(
        ...
        batch_size = 4
        x = jax.random.poisson(rng, lam=5.0, shape=(batch_size, input_dim)).astype(
            jnp.float32
        )
        ...
        dec_params = decoder.init(rng_dec, jnp.zeros((batch_size, latent_dim)), covariates=covs)
...
class TestVAERegistryAndModularAPI:
    ...
    def test_decoder_registry_has_simple(self):
        assert "simple" in DECODER_REGISTRY
        assert DECODER_REGISTRY["simple"] is SimpleDecoder

    def test_gaussian_encoder_is_abstract_encoder_subclass(self):
        assert issubclass(GaussianEncoder, AbstractEncoder)

    def test_simple_decoder_is_abstract_decoder_subclass(self):
        assert issubclass(SimpleDecoder, AbstractDecoder)
    ...
    def test_build_from_registry(self, rng, input_dim, latent_dim, hidden_dims):
        ...
        DecCls = DECODER_REGISTRY["simple"]
        encoder = EncCls(
            input_dim=input_dim,
            latent_dim=latent_dim,
            hidden_dims=hidden_dims,
...
class TestNumericalStability:
    ...
    def test_single_hidden_layer(self, rng):
        ...
        decoder = SimpleDecoder(
            output_dim=20, latent_dim=3, hidden_dims=[8]
        )
    ...
    def test_deep_architecture(self, rng):
        ...
        decoder = SimpleDecoder(
            output_dim=50, latent_dim=5, hidden_dims=dims
        )
    ...
    def test_large_latent_dim(self, rng):
        ...
        decoder = SimpleDecoder(
            output_dim=10, latent_dim=50, hidden_dims=[32]
        )
