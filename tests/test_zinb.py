...
import numpyro
from scribe.models import zinb_model, zinb_guide, zinb_log_likelihood
...
def test_inference_run(small_dataset, rng_key):
    ...
    assert results.model_type == "zinb"
...
def test_parameter_ranges(example_zinb_results):
    ...
    r_params = [param for param in example_zinb_results.params if param.startswith('r_')]
    for param in r_params:
        assert jnp.all(example_zinb_results.params[param] > 0)
        
    # Check p parameters - should be positive and appropriate for Beta distribution
    p_params = [param for param in example_zinb_results.params if param.startswith('p_')]
    for param in p_params:
        assert jnp.all(example_zinb_results.params[param] > 0)
        
    # Check gate parameters - should be positive and appropriate for Beta distribution
    gate_params = [param for param in example_zinb_results.params if param.startswith('gate_')]
...
def test_get_log_likelihood_fn():
    ...
    log_lik_fn = get_log_likelihood_fn("zinb")
    
    assert log_lik_fn == zinb_log_likelihood
...
def test_posterior_sampling(example_zinb_results, rng_key):
    ...
    assert posterior_samples['r'].shape == (n_samples, example_zinb_results.n_genes)
    assert posterior_samples['gate'].shape == (n_samples, example_zinb_results.n_genes)
...
def test_predictive_sampling(example_zinb_results, rng_key):
    ...
    pred_samples = example_zinb_results.get_predictive_samples(
        rng_key=random.PRNGKey(43),
        store_samples=True
    )
    
    expected_shape = (n_samples, example_zinb_results.n_cells, example_zinb_results.n_genes)
...
def test_ppc_sampling(example_zinb_results, rng_key):
    ...
    ppc_results = example_zinb_results.get_ppc_samples(
...
def test_get_distributions(example_zinb_results):
    ...
    scipy_dists = example_zinb_results.get_distributions(backend="scipy")
    assert 'p' in scipy_dists
    assert 'r' in scipy_dists
    assert 'gate' in scipy_dists
    
    # Get numpyro distributions
    numpyro_dists = example_zinb_results.get_distributions(backend="numpyro")
    assert 'p' in numpyro_dists
    assert 'r' in numpyro_dists
    assert 'gate' in numpyro_dists

def test_get_map(example_zinb_results):
    """Test getting MAP estimates."""
    map_estimates = example_zinb_results.get_map()
    
    assert 'p' in map_estimates
    assert 'r' in map_estimates
    assert 'gate' in map_estimates
    assert map_estimates['p'].shape == ()  # scalar
    assert map_estimates['r'].shape == (example_zinb_results.n_genes,)
    assert map_estimates['gate'].shape == (example_zinb_results.n_genes,)
    
def test_model_and_guide_retrieval(example_zinb_results):
    """Test that model and guide functions can be retrieved from results."""
    model, guide = example_zinb_results.get_model_and_guide()
    
    assert callable(model)
    assert callable(guide)

def test_log_likelihood_fn_retrieval(example_zinb_results):
    """Test that log likelihood function can be retrieved from results."""
    log_lik_fn = example_zinb_results.get_log_likelihood_fn()
...
def test_getitem_integer(example_zinb_results):
    ...
    assert subset.model_type == example_zinb_results.model_type
    
    # Check gene-specific parameters are correctly subset
    for param in example_zinb_results.params:
        if param.startswith('r_') or param.startswith('gate_'):
            # Gene-specific parameters should be subset
            orig_shape = example_zinb_results.params[param].shape
            if len(orig_shape) > 0 and orig_shape[0] == example_zinb_results.n_genes:
                assert subset.params[param].shape == (1,)

def test_getitem_slice(example_zinb_results):
...
def test_getitem_boolean(example_zinb_results):
...
def test_subset_with_posterior_samples(example_zinb_results, rng_key):
...
def test_compute_log_likelihood(example_zinb_results, small_dataset, rng_key):
    ...
    cell_ll = example_zinb_results.compute_log_likelihood(
        counts,
        return_by='cell'
    )
    
    # Check shape - should be (n_samples, n_cells)
    assert cell_ll.shape == (N_SAMPLES, example_zinb_results.n_cells)
    ...
    gene_ll = example_zinb_results.compute_log_likelihood(
        counts,
        return_by='gene'
    )
    
    # Check shape - should be (n_samples, n_genes)
    assert gene_ll.shape == (N_SAMPLES, example_zinb_results.n_genes)
    assert jnp.all(jnp.isfinite(gene_ll))

def test_compute_log_likelihood_batched(example_zinb_results, small_dataset, rng_key):
    ...
    full_ll = example_zinb_results.compute_log_likelihood(
        counts,
        return_by='cell'
    )
    
    # Compute with batching
    batched_ll = example_zinb_results.compute_log_likelihood(
        counts,
        batch_size=3,
        return_by='cell'
    )
