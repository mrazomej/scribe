...
def nbdm_mixture_model(
    ...
    # Reshape p to broadcast with r: (n_components,) -> (n_components, n_genes)
    p = p[:, None]
...
def nbdm_mixture_log_likelihood(
    ...
    p = jnp.array(p)[None, :, None]  # First convert scalar to array, then add dimensions
    ...
    if return_by == 'cell':
        if batch_size is None:
            ...
            if weights is None:
                ...
                log_probs = jnp.sum(
                    gene_log_probs, axis=-1) + jnp.log(mixing_weights)
            else:
                # Check that weights are of correct shape
                if len(weights) != n_genes:
                    raise ValueError(
                        "For return_by='cell', weights must be of shape (n_genes,)"
                    )
                # Sum over genes (axis=-1) to get (n_cells, n_components)
                log_probs = jnp.sum(
                    gene_log_probs * weights, axis=-1) + jnp.log(mixing_weights)
        else:
            ...
            for i in range((n_cells + batch_size - 1) // batch_size):
                ...
                if weights is None:
                    ...
                    log_probs = log_probs.at[start_idx:end_idx].set(
                        jnp.sum(batch_log_probs, axis=-1) + 
                    ...
                    # Store log probs for batch
                    log_probs = log_probs.at[start_idx:end_idx].set(
                        jnp.sum(batch_log_probs * weights, axis=-1) + 
                        jnp.log(mixing_weights)
                    )
        ...
        if batch_size is None:
            ...
            if weights is None:
                ...
                if len(weights) != n_cells:
                    raise ValueError(
                        "For return_by='gene', weights must be of shape (n_cells,)"
                    )
                # Sum over cells and add mixing weights
                log_probs = (jnp.sum(gene_log_probs * weights, axis=0).T + 
                             jnp.log(mixing_weights))  # (n_genes, n_components)
        else:
            ...
            for i in range((n_cells + batch_size - 1) // batch_size):
                ...
                if weights is None:
                    ...
                    # Sum over batch
                    log_probs += jnp.sum(batch_log_probs * weights, axis=0).T
