...
def likelihood_fn(params, n_obs=10_000, batch_size=10_000):
    """
    Computes the likelihood of the parameters given the Dirichlet-Multinomial
    model using TensorFlow Probability for GPU acceleration, with batching and CPU storage.
    ...
    Args:
        ...
        n_obs (int): The total number of observations to generate.
        batch_size (int): The number of observations to process in each batch.
    ...
    neg_binom = tfp.distributions.NegativeBinomial(total_count=r_vec, probs=p_hat)
    
    # Initialize list to store batch results on CPU
    batches = []
    ...
    for start in range(0, n_obs, batch_size):
        ...
        # Sample for this batch on GPU
        # with tf.device('/GPU:0'):
        u_vec_batch = neg_binom.sample(batch_size_actual)
        
        # Move batch to CPU and convert to NumPy
        u_vec_batch_cpu = u_vec_batch.numpy()
        
        # Add to list of batches
        batches.append(u_vec_batch_cpu)
    
    # Concatenate all batches on CPU
    u_vec = np.concatenate(batches, axis=0)
